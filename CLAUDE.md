...
- `docs/apple-silicon/handoff.md` — current state, source-code changes made, next-session checklist (its "Session" head carries the dated next-steps list).
- `docs/apple-silicon/metal-parity-roadmap.md` — canonical strategic direction: goal, workstreams A–E, gated milestones M0→M-V, anti-churn rules.
- `docs/apple-silicon/xbe-coverage-matrix.md` — live NV2A feature-surface burndown, regenerated by `scripts/apple-silicon/xbe-coverage-matrix.py` (do not hand-edit).
