...
- Shipped: cycle-12 §H.6 `image-blit` v0.3 bounded partial committed as `e8fba9e925` with renderer source clean (`hw/xbox/nv2a/pgraph/mtl/blit.c` diff = 0); Codex `changes`-mode validation completed (MAJOR ISSUES → all three findings adopted in-tree before commit); state artifacts durably closed.
