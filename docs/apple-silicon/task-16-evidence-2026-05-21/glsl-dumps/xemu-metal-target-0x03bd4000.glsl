/* color_target=0x03bd4000 */
/* regs: 0x00000012 0x00000000 0x3e830700 0xffff0070 0x00000111 0x00000081 0x00400000 0x00000000 0x00000000 */
...
vec4 v1 = inlineValue[0];
vec4 v2 = inlineValue[1];
layout(location = 3) in vec4 v3;
vec4 v4 = inlineValue[2];
vec4 v5 = inlineValue[3];
vec4 v6 = inlineValue[4];
vec4 v7 = inlineValue[5];
vec4 v8 = inlineValue[6];
layout(location = 9) in vec4 v9;
vec4 v10 = inlineValue[7];
vec4 v11 = inlineValue[8];
vec4 v12 = inlineValue[9];
vec4 v13 = inlineValue[10];
vec4 v14 = inlineValue[11];
vec4 v15 = inlineValue[12];
...
void main() {
  ...
  /* Slot 4: 0x00000000 0x0400001B 0x08361300 0x101807F8 */
    RCP(R1,x, R0.w);

  /* Slot 5: 0x00000000 0x0040001B 0x0400286C 0x2070E800 */
    MUL(oPos,xyz, R0, R1.x);

  /* Slot 6: 0x00000000 0x0020061B 0x0836106C 0x2070F818 */
    MOV(oD0,xyzw, v3);

  /* Slot 7: 0x00000000 0x0020121B 0x0836106C 0x2070F848 */
    MOV(oT0,xyzw, v9);

  /* Slot 8: 0x00000000 0x0020001B 0x0436106C 0x20701801 */
    MOV(oPos,w, R0);
...
pT0.xyz = (pT0.xyz * t0LogicalSize + vec3(4, 4, 4)) * vec3(0.007812, 0.007812, 0.062500);
...
v0.a = ab.a;
// Final Combiner
fragColor.rgb = max(v0.rgb, 0.0) + mix(vec3(max(vec4(0.0).rgb, 0.0)), vec3(max(vec4(0.0).rgb, 0.0)), vec3(max(vec4(0.0).rgb, 0.0)));
fragColor.a = max(v0.a, 0.0);
