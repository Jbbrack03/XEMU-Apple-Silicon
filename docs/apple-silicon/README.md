...
> **Start here (2026-06-15):** the canonical *direction* is
> [`metal-parity-roadmap.md`](metal-parity-roadmap.md) (goal, workstreams A–E,
> gated milestones M0→M-V, anti-churn rules). Live feature-coverage progress is
> [`xbe-coverage-matrix.md`](xbe-coverage-matrix.md) (generated by
> `scripts/apple-silicon/xbe-coverage-matrix.py`). Current state is
> [`handoff.md`](handoff.md); binding decisions are [`decision-log.md`](decision-log.md).
> Pre-2026-06 history for those two was archived under `_archive/` (verbatim).
> The narrative below predates the roadmap and is retained for context.
