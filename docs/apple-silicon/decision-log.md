...
## 2026-05-24 (cycle 42G real-Xbox deployment of cycle-42F R5-GREEN witness-only XBE — EXECUTED on `apple-silicon-performance`): bounded run-only + doc-only closeout slice deploying the cycle-42F R5-GREEN witness-only XBE (SHA `7831b0706850d961805d35f2f336c32866b2204dca9a470418895588e69f441c`, 155 648 B) to the real Xbox per the cycle-42E 18-step runbook with the cycle-42F interpretation delta; OUTCOME = `(eeprom=0xBC, count=0)` — the cleanest cycle-42F success signal

**Decision.** Cycle 42G executes the real-Xbox deployment of the cycle-42F R5-GREEN witness-only XBE per the runbook documented in `benchmark-runs/cycle42f-readback-marker-20260524T152642Z/SUMMARY.md` § "Recommended real-Xbox deployment runbook delta vs cycle 42E". 18-step sequence executed identically to cycle 42E with the cycle-42F interpretation delta substituted (widened expected post-run `eeprom.scratch.read` value set from `{0x00, 0xA4, 0xA6, 0xB0..0xBA, 0xBB}` to `{0x00, 0xA4, 0xA6, 0xB0..0xBA, 0xBB, 0xBC}`). A 00b pre-baseline ensure-agent step was added at the front because the cycle-42G starting state had `agent=false` (dashboard up post-cycle-42E close but no live agent), unlike cycle 42E's `agent=true` start. Logs and artifacts live at `benchmark-runs/cycle42g-realxbox-20260524T165715Z/` (gitignored per project convention). NO source changes; NO Codex pass (rule #15 doc-only carve-out — the cycle-42F build deployed here is Codex-R5-GREEN).

**Rationale.** The cycle-42F R5-GREEN implementation needed real-hardware classification to advance the cycle-22 narrative. The cycle-42F interpretation matrix's cleanest predicted success signal — `(eeprom=0xBC, count=0)` — required executing the bypass-body end-to-end on real hardware where the milestone-9 `wbinvd` + milestone-10 volatile readback could be exercised in their target environment. Postponing classification would have left cycle-42F's deploy-ready implementation idle without producing the disambiguation evidence its design purpose called for.

**Outcome = `(eeprom.scratch.read=0xBC, witness.scan-self count=0)`.** This is exactly the cleanest predicted success signal per the cycle-42F outcome matrix. The post-chainload signals were: `eeprom.scratch.read=0xBC tag=0xB stage_nib=0xC` + `witness.scan-self count=0 mapped_pages_seen=419` + `witness.scan=count=1 phys=0x03eb3000 reserved0=0 reserved1=0` (D-cycle-28 shape; 30+ consecutive observations across cycles 26..42G; cycle-23 lockstep contract intact). Full 256-byte EEPROM hex dump cross-check confirmed the last byte at offset 0xFF = `bc` (line ends `…0609 00bc`).

**What 0xBC PROVES (positive findings).** (i) The cycle-42D stage==6 bypass body executed end-to-end through all 11 milestones (0xB0..0xBA full completion). (ii) After the milestone-9 WTNS magic stamp + milestone-10 `wbinvd`, the volatile readback of `vp_c42d[0]` through the cached kseg0 alias `(phys | 0x80000000u)` observed `XBED_SELF_WITNESS_MAGIC` (0x534E5457). (iii) The subsequent `HalWriteSMBusValue(0xA8, 0xFF, FALSE, 0xBC)` marker write SUCCEEDED (otherwise the byte would have stayed at 0xBA). (iv) **Cycle-42E (β) "silent marker-write inflation past an unfired WTNS stamp" is RULED OUT** on the bypass-body + stamp-observability axis. (v) **Cycle-22 axis ADVANCES from cycle-42E's "PARTIALLY CONFIRMED on the bypass-body axis" → "CONFIRMED on the bypass-body + stamp-observability axis."** (vi) The cycle-42B custom pre-WinMainCRT entry-point thunk + cycle-42D 12-marker stage==6 fast-path bypass + cycle-42F post-stamp readback are all working as designed on real hardware.

**What 0xBC does NOT prove.** (a) The WTNS page survives the post-XBE chainload return to the dashboard (`MmPersistContiguousMemory` retention semantics on this BIOS revision for pre-WinMainCRT allocations remain unmeasured). (b) The page's phys falls inside the agent-side `witness.scan-self` enumeration window `[0x80010000, 0x84000000]`. (c) It does NOT formally distinguish (i) "page torn down inside the enumeration window before agent could read it" from (ii) "page survives but lands outside the enumeration window" — that disambiguation is the cycle-42H natural next slice.

**Reproducibility note (single-run).** The 0xBC observation is positive evidence and is NOT subject to the false-positive risks of count-based observations (which can be inflated by stale state); a 0xBC byte at off=0xFF can ONLY arise from the cycle-42F bypass body's milestone-10 readback-then-marker code path. A second confirmatory run is not required by the cycle-42F matrix for the `(0xBC, 0)` outcome — the matrix calls out reproducibility checks only for the unexpected `(0xBC, 1+)` row.

**Reproducibility wins carried forward.** `phys=0x03eb3000` deterministic kernel-pool reuse REPRODUCED across pre-baseline + post-deployment + final readbacks (30+ consecutive observations across cycles 26..42G). `mapped_pages_seen=419` REPRODUCED at every readback (cycles 26..42G). EEPROM non-volatility across the pre-chainload reboot CONFIRMED (`0xBA → reset → 0x00 → post-runxbe → 0xBC`). Recovery timing (t+19s reboot, t+21s post-runxbe) within cycles 40..42F 6..36s sampling band; consistent with the bypass body adding measurable wall time via the 0x2000 wipe loop + 11 SMBus writes + the new milestone-10 readback + 0xBC marker write. NOT load-bearing for outcome classification.

**Preservation.** ZERO host xemu source touched. ZERO `lib/xbed_self_witness.{c,h}` touched (cycle-42F R5-GREEN deployed bit-identically; SHA verified exact match before upload). ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact). ZERO `lib/lib.mk` touched. ZERO `oracle-agent/*` touched (cycle-39 v0.5 verbs decode the cycle-42F 0xBC marker via the cycle-42F interpretation table in `xbed_self_witness.h`, NOT via agent-side logic). ZERO `xbed_runtime.{c,h}` touched. ZERO `witness-only/main.c` / `Makefile` / `manifest.json` / `witness_only_crt0.c` touched (cycle-42B build mechanism intact). ZERO `nxdk/` source touched. ZERO `tools/xemu-capture/` source touched. ZERO `composite-record.sh` / `composite-preflight.sh` source touched. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail. Pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged. No new repo-root `.hermes_*` files created this cycle (the old pile was archived into `.claude/state/repo-root-hermes-archive/` during the cycle-42G pre-execution pause).

**Supersedes.** Nothing — cycle 42G is a new evidence row in the cycle-42 narrative chain, classifying the cycle-42F build per its documented outcome matrix.

**Recommended cycle 42H (Hermes's call).** The cycle-42F outcome matrix's residual in-window-torn-down vs out-of-window-surviving disambiguation is the natural next slice — option 1 of the original cycle-42E menu: broaden `oracle-agent/commands.c::cmd_witness_scan_self` phys-range enumeration. That is an oracle-agent source change and would re-trigger rule #15 (full Codex pass on the agent source). Mechanism: replace the fixed `[0x80010000, 0x84000000]` scan window with either (a) a full kseg0 sweep (`[0x80000000, KSEG0_END]`) or (b) a sized sweep matching the BIOS-reported usable RAM extent. If broadened scan finds the WTNS page → "(ii) page survives but lands outside the original enumeration window" CONFIRMED ⇒ cycle-22 axis FULLY CONFIRMED. If broadened scan still finds count=0 → "(i) page torn down inside the enumeration window before agent could read it" SUPPORTED but not formally proven; would require a separate `MmQueryAllocationSize`/equivalent probe to formally distinguish.

## 2026-05-24 (cycle 42F post-stamp in-process WTNS-magic readback discriminator — IMPLEMENTATION+BUILD+CODEX COMPLETE on `apple-silicon-performance`; real-Xbox deployment DEFERRED to next bounded slice): bounded implementation+build+Codex slice adding milestone marker `0xBC` to the cycle-42D stage==6 fast path so the cycle-42E `(eeprom.scratch.read=0xBA, witness.scan-self count=0)` collapse can be disambiguated on the bypass-body + stamp-observability axis WITHOUT an oracle-agent rebuild

**Decision.** Cycle 42F selects option 2 of the cycle-42E SUMMARY's three mutually-exclusive cycle-42F slice candidates: append milestone marker `XBED_SELF_WITNESS_C42D_M_POST_READBACK = 0xCu` (yielding post-run byte `0xBC = 0xB0 | 0xC`) to the cycle-42D stage==6 fast path, gated on a volatile in-process readback of `vp_c42d[0] == XBED_SELF_WITNESS_MAGIC` after the existing `self_witness_cycle42d_marker(C42D_M_FULL_COMPLETION)` call. ZERO oracle-agent source changes (option 1 was deferred as a Codex-mandatory oracle-agent change that would re-trigger rule #15). ZERO removal of the cycle-42D sticky-flag pre-set (option 3 was deferred as a behavior change to the cycle-39 EEPROM-write suppression contract). Source change in `scripts/apple-silicon/xbe-tests/lib/xbed_self_witness.{c,h}` (~+504 / -29 LOC; ~6 lines of new C body + ~290 LOC of in-source documentation + 5 R2-MED-adopted legacy-wording softenings). Real-Xbox deployment is the next bounded slice (cycle 42G).

**Rationale.** Cycle 42E closed with a NEW SIGNAL CLASS `(eeprom.scratch.read=0xBA, witness.scan-self count=0)` that collapses under cycle-42D R1 Honest-framing lower-bound semantics to TWO mutually-exclusive possibilities: (α) bypass body executed end-to-end through all 11 milestones (0xB0..0xBA) AND WTNS page no longer discoverable post-chainload, vs (β) silent marker-write inflation past an unfired WTNS stamp. (α) was strongly preferred but not formally proved by cycle 42E. Option 2 (this cycle) is the cleanest in-XBE-only disambiguator: a readback of `vp_c42d[0]` through the SAME kseg0 cached-mirror alias the WTNS magic stamp used, gated by the wbinvd flush at milestone 10. The wbinvd-then-volatile-load pair establishes — per Intel SDM Vol. 3A §2.8.6 / §14 — that the same-core same-virtual-address read sees the value the producer just stored to `vp_c42d[0]` via the SAME cached alias the agent's `witness.scan-self` consumer would later walk. That is sufficient to RULE OUT (β)-via-unfired-stamp on the bypass-body + stamp-observability axis. It is intentionally NARROWER than a "DRAM commit" or "post-chainload survival" claim — those are independent properties cycle 42F does NOT exercise.

**What 0xBC PROVES (narrowed per Codex R1+R3+R4 MEDs adopted).** Observing 0xBC at post-run advances the cycle-22 axis from cycle-42E's "PARTIALLY CONFIRMED on the bypass-body axis" → "CONFIRMED on the bypass-body + stamp-observability axis". RULES OUT cycle-42E (β)-via-unfired-WTNS-stamp. Combined with `count=0`, supports cycle-42E (α) "body completed end-to-end + post-chainload count=0 reflects a discoverability gap" reading — but does NOT by itself prove the FULL (α) case (the post-chainload in-window-torn-down vs out-of-window-surviving sub-question requires the cycle-42F alternative slice). Combined with `count=1+`, advances cycle-22 axis to FULLY CONFIRMED + reduces cycle-42E "WTNS page not discoverable post-chainload" framing to "intermittent visibility, not absent."

**What 0xBC does NOT prove.** (i) DRAM commit / post-chainload survival of the WTNS page (`MmPersistContiguousMemory` retention semantics for pre-WinMainCRT allocations remain unmeasured on THIS BIOS). (ii) The page's phys falls inside the agent's `witness.scan-self` enumeration window. (iii) The body executed past milestone 10 (the bypass returns IMMEDIATELY after the 0xBC marker write or after 0xBA if the readback fails — no further body code exists for cycle 42F to instrument). (iv) The 0xBC SMBus write itself succeeded — its NTSTATUS is discarded per cycle-42D R1.HIGH lower-bound contract; silent failure leaves the post-run byte at 0xBA even when the readback observed the magic. 0xBC is POSITIVE-ONLY evidence: observing 0xBC at post-run is monotonically stronger than observing 0xBA, but observing 0xBA cannot distinguish "readback returned non-magic" from "readback returned magic but 0xBC write failed silently."

**Codex 5 rounds.** R1 MED (over-claim "wbinvd-then-load forces read to go to memory" + "0xBC proves stamp committed to memory") ADOPTED via 3 documentation-only edits narrowing to in-process observability framing + Intel SDM citation; R1 also noted that the uncached RAM mirror in this codebase is `phys | 0xB0000000` (not `0xA0000000` as the R1 prompt asserted); the cached alias choice for this discriminator is still correct. R2 MED (5 pre-existing legacy comments in this file still attribute post-chainload visibility / survival to wbinvd / MmPersistContiguousMemory as measured fact: `xbed_self_witness.c:{35, 283, 325, 954}` + `xbed_self_witness.h:212`) ADOPTED via 5 documentation-only INTENT-vs-measured-fact edits. R3 MED (`.c:457` "observing 0xBC at post-run PROVES (α)" overshot what byte-only 0xBC establishes about the FULL (α) collapse) ADOPTED via 1 documentation-only narrowing edit. R4 MED (second spot at `.c:424` "(α) ... is the load-bearing reading" — same over-claim) ADOPTED via 1 documentation-only narrowing edit with same pattern as R3. **R5 GREEN — explicit "DEPLOY-READY"** with no findings; all R1+R2+R3+R4 sites closed; residual grep hits for "load-bearing reading" / "PROVES (α)" are attribution quotes describing prior wording (not live claims). ZERO C source body changes between R1 and R5 — all 4 adoption rounds were documentation-only. Codex marker at `.claude/state/codex-validate-last-run` refreshed to `2026-05-24T15:46:00Z` after R5.

**Build verification.** Clean nxdk `make` succeeds. `llvm-readobj --file-headers main.exe` reports `AddressOfEntryPoint=0x37D0` (cycle-42D R4-GREEN was 0x37C0; the 16-byte forward move reflects the cycle-42F readback compare + branch + new marker call site expansion in `xbed_self_witness_fire`). `llvm-objdump -d main.exe --start-address=0x13210 --stop-address=0x13830` confirms the new readback compare + conditional 0xBC marker call site at 0x13479..0x1348c (load `vp_c42d` from `-0x20(%ebp)`, deref `(%eax)`, `cmpl $0x534e5457` WTNS magic, `jne 0x13491` skip 0xBC on mismatch, `movl $0xc, (%esp)` push POST_READBACK milestone, `calll 0x13780` marker helper), with all cycle-42D milestone marker call sites + marker helper at 0x13780 + wbinvd helper at 0x137c0 + stages-!=6 path at 0x134B3 BIT-IDENTICAL to cycle-42D R4-GREEN. **Final cycle-42F R5-GREEN deployed-build SHA = `7831b0706850d961805d35f2f336c32866b2204dca9a470418895588e69f441c`** (155 648 B; archived in `benchmark-runs/cycle42f-readback-marker-20260524T152642Z/witness-only-cycle42f-r5-green.xbe`). Oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`.

**Preservation.** Cycle-23 lockstep (`lib/xbed_a4_witness.{c,h}`) UNTOUCHED. Cycle-29 stages-!=6 path UNTOUCHED at the C-body level (the `MmPersistContiguousMemory` comment was softened per R2 MED). Cycle-39 sticky-flag semantics for stages !=6 PRESERVED. Cycle-41c/41d guards inside the bypass REPLICATED unchanged. Cycle-42A multi-page allocation PRESERVED unchanged. Cycle-42B pre-WinMainCRT thunk (`witness-only/witness_only_crt0.c`) UNTOUCHED. Cycle-42D 11-marker sequence (0xB0..0xBA + 0xBB reuse) PRESERVED unchanged — the 0xBC marker is APPENDED after 0xBA and is NOT a renumbering of any existing milestone. The reuse-path 0xBB is UNTOUCHED (defensive: reuse-path readback would be reading a magic stamped by a PRIOR fire, not by the current one). All eight default-on Apple Silicon flags (project rule #11) UNTOUCHED (XBE-side instrumentation only, no host xemu source).

**Outcome.** Implementation deploy-ready (cycle 42G real-Xbox deployment is the next bounded slice — Hermes's call). Cleanest cycle-42F "cycle-22 axis CONFIRMED on the bypass-body + stamp-observability axis" signal = `(eeprom=0xBC, count=0)`. Strongest "(β)-via-unfired-stamp SUPPORTED" signal = `(eeprom=0xBA, count=0)` with NO 0xBC (could also be "readback observed magic but 0xBC SMBus write failed silently"; the asymmetry between observed-0xBA-after-42F and observed-0xBC-after-42F carries the new disambiguation power). "discoverability gap closed" signal = `(eeprom=0xBC, count=1+)` ⇒ cycle-22 FULLY CONFIRMED. Real-Xbox runbook delta vs cycle 42E documented in `benchmark-runs/cycle42f-readback-marker-20260524T152642Z/SUMMARY.md` (identical 18-step sequence; expected post-run EEPROM byte values widen from `{0x00, 0xA4, 0xA6, 0xB0..0xBA, 0xBB}` to `{0x00, 0xA4, 0xA6, 0xB0..0xBA, 0xBB, 0xBC}`).

**Supersedes.** Nothing — cycle 42F is a new evidence row in the cycle-42 narrative chain, not a reversal of a prior decision. The R2-MED-adopted legacy-wording softenings narrowed pre-existing comments in 5 spots to INTENT-vs-measured-fact framing without changing any C body or any logical claim about wbinvd / MmPersistContiguousMemory behavior; the documentation drift was a Codex-flagged internal inconsistency between cycle-42F's careful new honest-framing and pre-cycle-42F comments that had not been similarly narrowed.

**Recommended cycle 42G (Hermes's call).** Real-Xbox deployment of the cycle-42F R5-GREEN witness-only XBE. Reuse the cycle-42E 18-step deployment runbook with ONE interpretation change (full matrix in `xbed_self_witness.h` "Cycle-42F" Safety-notes subsection). If outcome = `(0xBC, count=0)`: cycle-22 axis advances to CONFIRMED on bypass-body + stamp-observability; cycle 42H investigates option 1 (broaden agent-side `witness.scan-self`) to disambiguate the residual in-window-torn-down vs out-of-window-surviving sub-question. If outcome = `(0xBA, count=0)` with NO 0xBC: (β)-via-unfired-stamp SUPPORTED (with the caveat that "0xBC write failed silently" is not formally distinguishable); cycle 42H investigates whether a second-source corroboration of the WTNS stamp can be added (e.g. a second-marker readback at a different offset within the WTNS page). If outcome = `(0xBC, count=1+)`: cycle-22 FULLY CONFIRMED; cycle 42H is a reproducibility check.

## 2026-05-24 (cycle 42E real-Xbox deployment of cycle-42D R4-GREEN stage-6 marker-bypass XBE — EXECUTED on `apple-silicon-performance`): bounded run-only + doc-only closeout slice deploying the cycle-42D R4-GREEN witness-only XBE (SHA `d69f23fae70bacf26c82c7e2e96e9a08a7175de9950142a2321f91ef713a3093`, 155 648 B) to the real Xbox and classifying the outcome via the cycle-42D milestone marker matrix

**Decision.** Cycle 42E deploys cycle-42D's R4-GREEN witness-only XBE to the real Xbox per the runbook documented in `benchmark-runs/cycle42d-stage6-marker-bypass-20260524T130903Z/SUMMARY.md` §"Recommended real-Xbox deployment runbook delta vs cycle 42C". 18-step sequence executed identically to cycle 42C with the cycle-42D interpretation matrix substituted: reboot → FTP-upload (`--overwrite`) → `ensure-agent` → `unsafe.enable` → `eeprom.scratch.reset` → pre-baselines → `runxbe E:\Apps\witness-only\default.xbe` → poll dashboard FTP recovery → post-chainload `ensure-agent` → final `witness.scan` + `witness.scan-self` + `eeprom.scratch.read` + full `eeprom` hex dump cross-check. Logs and artifacts live at `benchmark-runs/cycle42e-realxbox-20260524T143801Z/` (gitignored per project convention).

**Rationale.** Cycle 42D closed with implementation+build+Codex deploy-ready but explicitly DEFERRED real-Xbox deployment to the next bounded slice (Hermes's call). Cycle 42E is exactly that deployment. The cycle-42D matrix was designed to be self-classifying: the post-run EEPROM byte (a LOWER BOUND on milestones reached per Codex R1.HIGH adoption) tells how far the bypass body progressed; the joint `witness.scan-self (count, reserved0, reserved1)` either confirms full success or disambiguates the residual ambiguity at the highest reached milestone.

**Outcome — NEW SIGNAL CLASS `(eeprom.scratch.read=0xBA, witness.scan-self count=0)`.** The `(0xBA, count=0)` shape is NOT directly enumerated in the cycle-42D matrix (`xbed_self_witness.h` lines 636-654 only enumerate `(0xBA, count >= 1)` as "best success / STRONG cycle-22 CONFIRMED"); under cycle-42D R1 Honest-framing lower-bound semantics, it collapses to TWO enumerated possibilities — **(α) bypass body executed end-to-end through all 11 milestones (0xB0..0xBA), WTNS page no longer discoverable post-chainload** vs **(β) silent marker-write inflation where the SMBus helper succeeded on a later milestone while a strictly-earlier body step faulted silently**. (α) is the strongly-preferred reading because the marker helper is a one-shot SMBus write to a stable 24LC02 device and (β)'s "SMBus drops earlier writes but accepts later ones" is vanishingly unlikely on a stable post-cold-boot console.

**What this signal proves at HIGH confidence regardless of α vs β.**

1. Entry-point override + thunk still execute correctly on real hardware (re-confirms cycle 42C); 0xB0 marker overwrote thunk's 0xA6 defensive pre-write, so the bypass was entered.
2. `HalWriteSMBusValue` does NOT fault from pre-WinMain context — eleven independent SMBus write call sites (0xB0..0xBA) landed and persisted.
3. **Cycle-22 hypothesis (a) "pre-allocator vsnprintf-pre-libc fault" RULED OUT.** The bypass skips every `xbed_host_log_writef`/`vsnprintf` call AND we observe the 0xBA marker which sits AFTER every body step in the cycle-42A redesign sequence (alloc + guards + persist + wipe + WTNS stamp + reserved0/1 + wbinvd). Under (α) the alternative pre-libc-allocator-fault hypothesis is also RULED OUT (`MmAllocateContiguousMemory(0x2000)` returned non-NULL from strict pre-WinMain context, both phys-range guards passed, page-alignment guard passed, `MmPersistContiguousMemory` returned, page wipe completed, WTNS stamp ran, reserved0/1 stamp + `wbinvd` ran).
4. The cycle-39 sticky-flag pre-set worked as designed — NONE of the five WTNS-shim fires (stage 6 from thunk + stage 4 from `.CRT$XXC` + stage 5 from `.CRT$XCU` + stage 1+3 from in-main WTNS1/WTNS2) overwrote the 0xBA marker with `0xA4`.

**Cycle-22 hypothesis state.** ADVANCES from "NOT advanced" (cycle 42C) → "PARTIALLY CONFIRMED on the bypass-body axis" (cycle 42E). The residual `count=0` observation is a *discoverability* gap (likely either `MmPersistContiguousMemory`'s persistence semantics on this BIOS revision not extending the WTNS page across the post-runxbe chainload return to dashboard, OR the persisted phys page lands outside the kseg0 range that `witness.scan-self` enumerates), NOT a calling-context gap.

**Final signals.** `eeprom.scratch.read=0xBA tag=0xB stage_nib=0xA` + `witness.scan-self count=0 mapped_pages_seen=419` + `witness.scan=count=1 phys=0x03eb3000 reserved0=0 reserved1=0` (D-cycle-28 shape; 29+ consecutive observations across cycles 26..42E; cycle-23 lockstep intact). Full `eeprom` hex dump confirmed last byte = `ba`. Recovery timing t+8s on reboot 1, t+26s on post-runxbe (within cycles 40..42D 6..36s sampling band; t+26s slightly above the t+18..t+20s seen on cycles 41e..42C — consistent with the bypass body adding measurable wall time via the 0x2000 wipe loop + 11 SMBus writes).

**ZERO source / build state changes; pure run-only + doc-only closeout.** ZERO host xemu source touched; ZERO `lib/xbed_self_witness.{c,h}` touched; ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact); ZERO `lib/lib.mk` touched; ZERO `oracle-agent/*` touched (cycle-39 v0.5 verbs intact); ZERO `xbed_runtime.{c,h}` touched; ZERO `witness-only/main.c` / `Makefile` / `manifest.json` / `witness_only_crt0.c` touched (cycle-42D build deployed bit-identically); ZERO `nxdk/` source touched; ZERO `tools/xemu-capture/` source touched; ZERO `composite-record.sh` / `composite-preflight.sh` source touched. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged; pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged; 25+ pre-existing untracked `.hermes_*` files at repo root preserved un-staged.

**Codex.** No Codex pass (rule #15 doc-only carve-out; the cycle-42D R4-GREEN build deployed here was Codex-R4-GREEN).

**Recommended cycle 42F (Hermes's call) — mutually exclusive bounded slices.**

1. Broaden `oracle-agent/commands.c::cmd_witness_scan_self`'s phys-address enumeration (Codex-mandatory; oracle-agent source change; promotes cycle-22 to FULLY CONFIRMED on count→1+).
2. Add a "post-stamp readback" EEPROM marker 0xBC in the cycle-42D bypass that reads back the WTNS magic from the just-stamped page and stamps 0xBC if it survived in-process (in-XBE-only change, one Codex pass; cleanest next slice — disambiguates (α) vs (β) without an oracle-agent rebuild and without giving up the cycle-22 lockout property).
3. Remove the cycle-42D sticky-flag pre-set so the cycle-39 EEPROM write CAN fire on later stages-!=6 fires (discriminator for whether `WinMainCRTStartup` returned).

**Supersedes.** Nothing — cycle 42E is a new evidence row in the cycle-42 narrative chain, not a reversal of a prior decision.

---

## 2026-05-24 (cycle 42D stage-6 pre-libc-safe milestone marker bypass — IMPLEMENTATION+BUILD+CODEX COMPLETE on `apple-silicon-performance`; real-Xbox deployment DEFERRED to next bounded slice): bounded implementation+build+Codex sub-slice in `scripts/apple-silicon/xbe-tests/lib/xbed_self_witness.{c,h}` adding a `stage == 6` conditional fast path at the top of `xbed_self_witness_fire` that bypasses the suspected `xbed_host_log_writef → vsnprintf` pre-libc fault site identified by cycle 42C and emits distinct EEPROM marker bytes (high-nibble 0xB, low-nibble milestone index 0..0xB) at every shim-internal milestone so the next real-Xbox run can identify exactly how far the pre-WinMain fire progressed

**Decision.** Cycle 42D adds a `stage == 6` conditional fast path at the top of `xbed_self_witness_fire` (in `scripts/apple-silicon/xbe-tests/lib/xbed_self_witness.c`) that (i) sets `s_eeprom_scratch_attempted = 1` as the FIRST shim action (atomic 1-instruction mov; locks out cycle-39 EEPROM writes for any later stages-!=6 fire — preventing a `.CRT$XXC` stage=4 fire from overwriting cycle-42D markers with 0xA4), (ii) writes EEPROM marker 0xB0 (cycle-42D entry tag) via a new `self_witness_cycle42d_marker(uint8_t milestone)` static-inline `__attribute__((no_stack_protector))` helper that calls `HalWriteSMBusValue(0xA8, 0xFF, FALSE, 0xB0 | (milestone & 0x0F))` directly and discards NTSTATUS, (iii) skips every `xbed_host_log_writef` / `xbed_host_log_write` call (the suspected vsnprintf-pre-libc fault site), (iv) replicates the cycle-42A 2-page allocation + cycle-41c symmetric phys-range guards + cycle-41d page-alignment guard + `MmPersistContiguousMemory` + page wipe + cycle-29 WTNS stamp + `wbinvd` — emitting milestone markers 0xB1..0xBA after each major step (+ 0xBB for idempotent reuse), (v) returns early so control does NOT fall through to the existing stages-!=6 host-log tail. Stages != 6 unchanged.

**Why a stage==6 bypass.** Cycle 42C closed with INCONCLUSIVE `(eeprom=0xA6, count=0)`. The byte-stayed-at-0xA6 evidence RULED OUT hypothesis (b) "allocator-then-post-guard rejection"; live failure modes narrowed from {a, b, c} to {a, "post-cycle-39-write-site fault that prevents the sticky flag from being set"}, dominated by leading hypothesis (a) "pre-allocator fault inside the shim's pre-libc context — most likely `xbed_host_log_writef → vsnprintf`". The cycle-42D bypass is the cheapest in-shim discriminator that can split (a) into sub-cases: it skips the suspected fault site entirely and adds finer-grained EEPROM markers between every kernel call inside the shim. A full rewrite of the cycle-29 shim would re-validate cycles 29..42A's preserved invariants from scratch (high blast radius); the conditional fast path keeps the stages-!=6 path bit-identical.

**Marker scheme.** High-nibble 0xB (distinct from cycle-39 0xA0..0xAF stage-byte namespace); low-nibble milestone index 0..0xB. 11 milestones in execution order (entry 0xB0 / pre-alloc 0xB1 / post-alloc 0xB2 / post-phys 0xB3 / post-lower-guard 0xB4 / post-upper-guard 0xB5 / post-align-guard 0xB6 / post-persist 0xB7 / post-wipe 0xB8 / post-WTNS-stamp 0xB9 / full-completion 0xBA) plus 0xBB for the not-expected idempotent-reuse case. Each marker overwrites the previous. **Honest framing (Codex R1.HIGH adopted):** every marker write discards NTSTATUS, so the post-run EEPROM byte identifies the highest milestone whose marker write SUCCEEDED — a LOWER BOUND on body progress, NOT an exact "execution died here" boundary. The cycle-42D real-Xbox interpretation must enumerate ALL the joint possible meanings each (byte, count) shape collapses to and use the `witness.scan-self (reserved0, reserved1)` joint readback as disambiguator where possible. Full matrix in `xbed_self_witness.h` "Cycle-42D" Safety-notes subsection.

**Why high-nibble 0xB (not extending 0xA0..0xAF).** Cycle-39 stage byte uses high-nibble 0xA with low-nibble = stage code (we already use stages 1, 3, 4, 5, 6 → 0xA1, 0xA3, 0xA4, 0xA5, 0xA6). High-nibble 0xB reserves the adjacent nibble; 0xC0..0xFF stays free for future cycle-42E+ markers.

**Why NOT a new EEPROM offset.** Offset 0xFF is the single byte the agent's `eeprom.scratch.read` verb is wired to (cycle-39 design). Adding a new offset would require oracle-agent verb changes (rule-#15 trigger). The 1-byte resolution at 0xFF is sufficient for the 12-milestone marker set.

**Why NOT add a vsnprintf-free host-log channel.** The cycle-15 `xbed_host_log_write` (non-formatting) already exists and does NOT call vsnprintf (just outb to port 0xE9). We chose to skip ALL host-log calls in cycle 42D because: (i) the host-log channel surfaces only on xemu (not real Xbox); cycle-42D's target is the real-Xbox calling-context discriminator, and EEPROM markers ARE durable across the chainload; (ii) calling outb from pre-libc context is plausibly safe but unverified on this hardware, and EEPROM via HalWriteSMBusValue has been EMPIRICALLY validated to work from pre-WinMain context (cycle-42C thunk pre-write landed and persisted); (iii) keeping the bypass minimal (zero non-kernel-export calls) reduces the surface area where a pre-libc fault could mask the diagnostic signal. The cycle-29 stages-!=6 path still emits the existing host-log lines for xemu-side debugging.

**`__attribute__((no_stack_protector))` on both `xbed_self_witness_fire` and the marker helper.** The stage==6 dispatch may run before `__security_init_cookie`. The standard compiler-inserted cookie check would compare against `__security_cookie` while it still holds its linker-initialized value. Mirrors the cycle-42B thunk's existing attribute on `witness_only_pre_winmain_crt_startup`.

**Codex 4-round validation.** R1 — HIGH "cycle-42D EEPROM table over-claims one-byte uniqueness because every marker write is best-effort and its NTSTATUS is discarded; the byte is a LOWER BOUND on milestones reached" with three concrete row examples (sticky-flag-set-but-marker-write-failed; alloc-OK-but-next-marker-failed; stamp-OK-but-final-marker-failed); ADOPTED via documentation-only rewrite of the interpretation table + marker helper docstring + per-line bypass comments. → R2 — MED on two residual exact-boundary summary lines (`.c:180` + `.h:477`); ADOPTED via two softening edits. → R3 — MED on a third residual summary line (`.h:733`); ADOPTED via further softening edit. → R4 — **GREEN — explicit "DEPLOY-READY"** with no findings, no new MED/HIGH issues, no remaining summary text reintroducing the exact-last-milestone over-claim. All hard findings adopted across 4 rounds via documentation-only edits; ZERO C source body changes between R1 and R4 (function body bit-identical from first successful build to R4-GREEN). Codex marker at `.claude/state/codex-validate-last-run` refreshed to `2026-05-24T14:07:40Z` after R4.

**Build verification.** Clean `make`; `llvm-readobj --file-headers` reports `AddressOfEntryPoint=0x37C0` (cycle-42B thunk moved from 0x3510 because cycle-42D code in xbed_self_witness.c expanded that function by ~470 bytes — expected). `llvm-objdump` of `xbed_self_witness_fire` at 0x13210 confirms: stage==6 dispatch (`cmpl $0x6, 0x8(%ebp)` + `jne 0x13485`); sticky-flag-first ordering (`movl $0x1, 0x34e68` BEFORE first HalWriteSMBusValue); 11 milestone marker call sites at expected sequential positions; marker helper at 0x13770 encoding `(0xB0 | (milestone & 0x0F))` via `andl $0xf, %eax` + `orl $0xb0, %eax` before `calll *0x337c8` (HalWriteSMBusValue import thunk — same slot the cycle-42B thunk uses); cycle-42A numerics (0x2000 alloc + 0x10000 lower guard + 0x4000000 upper guard + 0xFFF align mask + 0x2000 persist + 0x800 page-wipe bound); WTNS magic 0x534E5457 + VERSION 1; reserved0 stamp uses `orl $0xa4000000, %ecx` (cycle-23 lockstep 0xA4 tag preserved); stages-!=6 path at 0x13485 starts with the existing `xbed_host_log_writef("xbed_self_witness: enter stage=%u", ...)` call — UNCHANGED. **Final cycle-42D R4-GREEN deployed-build SHA = `d69f23fae70bacf26c82c7e2e96e9a08a7175de9950142a2321f91ef713a3093`** (155 648 B; oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`).

**Outcome.** Implementation deploy-ready (cycle 42E real-Xbox deployment is the next bounded slice — Hermes's call). Cleanest cycle-42D "calling-context CONFIRMED" signal = `(0xBA, count=1, reserved0 low byte = 0xA3, reserved1 = 5)` ⇒ k=1 (pre-WinMain was the first successful allocator AND all five WTNS-shim fires reached). Strongest "calling-context REJECTED at allocator" signal = `(0xB1, count=0)` ⇒ stage=6 reached the allocator call AND the allocator returned NULL ⇒ cycle-22 hypothesis (c) "true allocator rejection" CONFIRMED at strictest pre-CRT context. Other shapes have bounded multi-meaning interpretations per the Honest-framing matrix in `xbed_self_witness.h`. Real-Xbox runbook delta vs cycle 42C documented in `benchmark-runs/cycle42d-stage6-marker-bypass-20260524T130903Z/SUMMARY.md` (identical sequence; expected post-run EEPROM byte values widen from {0x00, 0xA4, 0xA6} to {0x00, 0xA4, 0xA6, 0xB0..0xBA, 0xBB}; cycle-42B joint `(reserved0_last_stage, reserved1)` table still applies for `count >= 1` cases; `(0xB9, 1)` shape requires the joint `(reserved0, reserved1)` readback as disambiguator).

**Preservation contract.** Cycle-23 lockstep (`lib/xbed_a4_witness.{c,h}`) UNTOUCHED. cycle-29 stages-!=6 path UNTOUCHED. cycle-39 sticky-flag semantics for stages !=6 PRESERVED (the unchanged path still checks `!s_eeprom_scratch_attempted`; cycle-42D pre-sets the flag from stage=6 ONLY when stage==6 ran first — by design, since cycle-42D markers replace the cycle-39 signal when stage==6 runs). cycle-41c/41d guards REPLICATED inline in the stage==6 bypass with identical numeric thresholds AND PRESERVED unchanged in the stages-!=6 path. cycle-42A multi-page (0x2000) REPLICATED inline AND PRESERVED unchanged. cycle-42B pre-WinMainCRT thunk UNTOUCHED — the thunk's defensive 0xA6 pre-write still runs FIRST; cycle-42D's 0xB0 marker overwrites that pre-write on entry to the bypass. If the bypass is never entered for any reason, EEPROM stays at 0xA6 — same INCONCLUSIVE shape as cycle 42C, now distinguishable from cycle-42A (0xA4) and from cycle-42D-success (0xBA) signals. ZERO host xemu source touched; ZERO `lib/xbed_a4_witness.{c,h}` / `lib/lib.mk` / `oracle-agent/*` / `xbed_runtime.{c,h}` / `witness-only/*` / `nxdk/` / `tools/xemu-capture/` / `composite-record.sh` / `composite-preflight.sh` touched. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail; pre-existing untracked `composite_preflight.py` + 25+ `.hermes_*` files PRESERVED un-staged.

**Recommended cycle 42E (Hermes's call).** Real-Xbox deployment of the cycle-42D witness-only XBE (SHA `d69f23fa…`) per the runbook delta above. Classify per the cycle-42D interpretation matrix. If `(0xBA, count=1, reserved0 low byte = 0xA3, reserved1 = 5)`: cycle-22 calling-context CONFIRMED; cycle 43 investigates which pre-WinMain property is operative (no-cookie / no-TLS / no-libc / no-thread / different-stack-frame). If `(0xB1, count=0)`: cycle-22 hypothesis (c) CONFIRMED at strictest pre-CRT context; cycle 43 investigates why the kernel allocator rejects the cycle-29 tuple from any pre-`.CRT$X*` calling context. Other shapes have specific cycle-43 follow-up paths per the marker table.

## 2026-05-24 (cycle 42C real-Xbox deployment of cycle-42B pre-WinMainCRTStartup thunk — EXECUTED on `apple-silicon-performance`): bounded run-only + doc-only closeout slice deploying the Codex-R5-GREEN cycle-42B witness-only XBE (SHA `3cc670fb…`) to real hardware; OUTCOME = INCONCLUSIVE per cycle-42B SUMMARY runbook row (c) — (eeprom=0xA6, witness.scan-self count=0); narrows live failure modes from {a, b, c} to {a, "post-cycle-39-write-site fault that prevents the sticky flag from being set"}; hypothesis (b) post-allocation-guard rejection is RULED OUT by the byte-stayed-at-0xA6 evidence

**Decision.** Cycle 42C deploys the cycle-42B witness-only XBE to real Xbox hardware and classifies the outcome per the joint (reserved0_last_stage, reserved1) discriminator table documented in the cycle-42B SUMMARY. Pure run-only + doc-only slice — no source changes; ZERO Codex pass (project rule #15 doc-only carve-out: the cycle-42B build deployed here was Codex-R5-GREEN; the cycle-42C diff is a SUMMARY.md + canonical-doc update with no implementation changes).

**Why run-only without Codex.** Project rule #15 trigger #2 ("non-trivial uncommitted code in `xemu-fork/`") does not fire because cycle 42C ships ZERO C / Makefile / script source changes. The only files produced are a per-run SUMMARY.md under `benchmark-runs/cycle42c-realxbox-20260524T122656Z/` (gitignored) and the canonical-doc updates in `handoff.md` + `decision-log.md` + the orchestration-state quartet. The Stop hook's >30-line aggregate-diff threshold is exceeded by the doc updates (which themselves are doc-only, not source), but the hook's prose-skip carve-out documented at `xemu-fork/CLAUDE.md` rule #15 ("doc-only changes" skip automatically) applies. The cycle-42B Codex 5-round review fully covers the deployed artifact.

**Deployment runbook executed (per cycle-42B SUMMARY § "Real-Xbox deployment runbook (delta vs cycle 42A)").** 18 logged steps under `benchmark-runs/cycle42c-realxbox-20260524T122656Z/`:

- **00** baseline status: `ping=true ftp=false agent=true` (agent v0.5 still resident from cycle 42B start state).
- **01a-c** pre-baseline scans: `witness.scan` D-cycle-27 phys=0x03eb3000 reserved0=0 reserved1=0; `witness.scan-self count=0 mapped_pages_seen=419`; `eeprom.scratch.read byte=0xA4` (cycle-42A leftover — cycle 42B did not deploy to hardware).
- **02** reboot at 2026-05-24T12:27:17Z.
- **03** dashboard FTP back at t+18s.
- **04** cycle-42B SHA recap (`3cc670fb…`) ✓.
- **05** FTP-upload witness-only with `--overwrite` (uploaded=1 skipped=0 failed=0).
- **06** ensure-agent (v0.5 re-launched).
- **07** unsafe.enable; **08** eeprom.scratch.reset → byte=0x00; **09** post-reset eeprom.scratch.read = byte=0x00 baseline confirmed.
- **10** witness.scan-self count=0; **11** witness.scan D-cycle-27 phys=0x03eb3000.
- **12** runxbe `E:\Apps\witness-only\default.xbe` at 2026-05-24T12:28:40Z.
- **13** dashboard FTP back at t+20s.
- **14** ensure-agent post-chainload (v0.5 re-launched).
- **15** final witness.scan D-cycle-27 phys=0x03eb3000 reserved0=0 reserved1=0 (cycle-23 lockstep intact across the post-runxbe reboot).
- **16** final witness.scan-self `count=0 mapped_pages_seen=419`.
- **17** final eeprom.scratch.read `byte=0xA6 tag=0xA stage_nib=0x6`.
- **18** full `eeprom` hex dump cross-check confirmed last byte = `a6` (line ends `…0900a6`).

**Outcome classification per the cycle-42B SUMMARY runbook.** Observed shape = `(eeprom.scratch.read=0xA6, witness.scan-self count=0)` matches the **INCONCLUSIVE** outcome row enumerated in the cycle-42B SUMMARY's "Recommended next-cycle" section.

**What this signal proves (HIGH confidence).** (i) The PE `AddressOfEntryPoint=0x3510` redirection to `witness_only_pre_winmain_crt_startup` ran on real hardware; the thunk's first executable code reached its SMBus write. (ii) `HalWriteSMBusValue(0xA8, 0xFF, FALSE, 0xA6)` does NOT fault from the strict pre-WinMain context — the defensive pre-write landed and persisted across the post-runxbe reboot. (iii) The cycle-39 sticky `s_eeprom_scratch_attempted` flag inside the shim was NOT set during this run: if any of the five WTNS-shim fires had executed far enough to reach the cycle-39 EEPROM write site, the byte would have been overwritten with `0xA0|stage_of_first_fire` = `0xA4` for stage 4 (per the cycle-39 sticky-flag-on-entry contract). The fact that the byte stayed at `0xA6` means **none of the five WTNS-shim fires reached the cycle-39 EEPROM write site** inside the shim. (iv) No WTNS magic landed on any allocated page (`count=0`).

**What this signal CANNOT distinguish, and the (b)-elimination subtlety.** Per Codex R2 adoption of the cycle-42B documentation, the (count=0, EEPROM=0xA6) outcome could in principle reflect any of {(a) pre-allocator fault — e.g. `xbed_host_log_writef → vsnprintf` from pre-libc context; (b) allocator-then-post-guard rejection — cycle-41c phys-range or cycle-41d alignment guard rejects + frees + returns 0; (c) true allocator rejection in pre-WinMain context propagating identically through later fires}. HOWEVER, the byte-stayed-at-0xA6 observation rules out (b): the post-allocation guards in cycle-41c/d preserve the sticky-flag setter sequence (the sticky flag is set on shim ENTRY, before host-log, before the allocator call); if any fire reached the allocator and its post-guards, the sticky flag would have been set and the cycle-39 EEPROM write site would have fired with `0xA4`. The fact that EEPROM stayed at the defensive `0xA6` value means no fire even reached the sticky-flag setter line. **Live cycle-42B failure modes narrow from {a, b, c} to {a, "post-cycle-39-write-site fault that prevents the sticky flag from being set on entry"}.**

**Strengthened leading hypothesis.** The remaining live mode (a) is **pre-allocator fault inside the shim's pre-libc-init context** — most likely `xbed_host_log_writef → vsnprintf` (which sits in the shim's hot path BEFORE the cycle-39 EEPROM write site at `xbed_self_witness.c`). The alternative "post-cycle-39-write-site fault that prevents the sticky flag from being set on entry" is equivalent to "the very first instruction inside the shim faults before any user-code executes" — implausible because the shim entry code is bog-standard C with no pre-libc dependencies until `xbed_host_log_writef`. The empirically-supported leading hypothesis is therefore (a) vsnprintf-pre-libc fault inside the shim's earliest host-log call.

**Cycle-22 axis state after cycle 42C.** NOT advanced. The cycle-42B discriminator did not deliver a clean ⟨calling-context CONFIRMED⟩ or ⟨calling-context RULED OUT⟩ signal because the shim itself faults from the pre-WinMain context BEFORE reaching the allocator. The cycle-22 calling-context axis is still live; cycle 42D would need to bypass the suspected fault site to either prove or disprove calling-context as the failing axis.

**Recommended cycle 42D (Hermes's call).** Stage-6-specific shim bypass behavior: when called from `stage=6`, the shim should (i) skip every `xbed_host_log_writef` call (the suspected vsnprintf-pre-libc fault site), and (ii) write additional EEPROM marker bytes at each shim-internal decision point — e.g. `0xB0|stage_nib` at shim entry, `0xC0|stage_nib` pre-allocator-call, `0xD0|stage_nib` post-allocator-success, `0xE0|stage_nib` post-guards-pass, `0xF0|stage_nib` pre-cycle-39-write — so the post-run EEPROM byte uniquely identifies which decision point was reached. Bounded scope: ~30-50 LOC addition to `lib/xbed_self_witness.c` conditional on `stage==6`; ZERO changes to other shim consumers; ZERO changes to oracle-agent. Codex 1-2 rounds expected.

**Reproducibility wins.** `phys=0x03eb3000` deterministic kernel-pool reuse reproduced across baseline + post-deploy + final readbacks (28+ consecutive observations across cycles 26..42C); `mapped_pages_seen=419` reproduced at every readback; EEPROM non-volatility across the pre-chainload reboot CONFIRMED (0xA4 → reset → 0x00 → post-runxbe → 0xA6); recovery timing (t+18s reboot 1, t+20s post-runxbe) within the cycles-40..42A 6..36s sampling band — NOT load-bearing.

**ZERO scope drift.** ZERO host xemu source touched; ZERO `lib/xbed_self_witness.{c,h}` touched; ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact); ZERO `lib/lib.mk` touched; ZERO `oracle-agent/*` touched (cycle-39 v0.5 verbs intact); ZERO `xbed_runtime.{c,h}` touched; ZERO `witness-only/main.c` / `Makefile` / `manifest.json` / `witness_only_crt0.c` touched (cycle-42B build deployed bit-identically); ZERO `nxdk/` source touched; ZERO `tools/xemu-capture/` source touched; ZERO `composite-record.sh` / `composite-preflight.sh` source touched. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail; pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged; 25+ pre-existing untracked `.hermes_*` files at repo root preserved un-staged. Evidence-only files at `benchmark-runs/cycle42c-realxbox-20260524T122656Z/{SUMMARY.md, 00..18-*.{txt,json}}` are gitignored per project convention.

---

## 2026-05-24 (cycle 42B custom pre-WinMainCRTStartup entry-point thunk — IMPLEMENTATION+BUILD+CODEX COMPLETE on `apple-silicon-performance`; real-Xbox deployment DEFERRED to next bounded slice): NEW `witness_only_crt0.c` thunk fires the cycle-29 self-witness shim BEFORE nxdk's WinMainCRTStartup; cxbe inspection shows the prompt's "modify nxdk/tools/cxbe" framing was unnecessary; smaller-blast-radius per-XBE `-entry:` link flag suffices

**Decision.** Cycle 42B executes the calling-context-axis discriminator — the only live cycle-22 axis after cycle-42A exhausted the in-XBE / lib-only candidate set (cache policy + address range + alignment + entry-point variant + size all G0(c) PERSISTS). NEW file `scripts/apple-silicon/xbe-tests/witness-only/witness_only_crt0.c` defines `witness_only_pre_winmain_crt_startup` which (1) defensively writes EEPROM offset 0xFF = `0xA0 | 6 = 0xA6` via `HalWriteSMBusValue` as its very first instruction (defense against vsnprintf-pre-libc fault inside the cycle-29 shim's `xbed_host_log_writef`); (2) calls `xbed_self_witness_fire(stage=6=WITNESS_ONLY_STAGE_PRE_WINMAIN_CRT)` with the cycle-29 self-witness shim UNCHANGED; (3) calls nxdk's standard `WinMainCRTStartup` so the rest of the runtime is bit-identical to a non-cycle-42B nxdk XBE. The thunk is selected as the PE entry point via `NXDK_LDFLAGS += -entry:witness_only_pre_winmain_crt_startup` in `witness-only/Makefile` (+12 / -0). The thunk runs strictly BEFORE nxdk's `__security_init_cookie` + TLS-size computation + `_PDCLIB_xbox_libc_init` + `_PDCLIB_xbox_run_pre_initializers` (.CRT$XX* walk) — i.e. the absolute earliest legitimate user-code execution context inside the XBE.

**Why the smaller-blast-radius `-entry:` flag instead of modifying cxbe.** Cycle-41e + 42A closure docs framed candidate B as "custom XBE-header callback before `_start` via `nxdk/tools/cxbe/` modifications." This session's design-discovery inspection of `nxdk/tools/cxbe/Xbe.cpp:222` showed cxbe does NOT invent the entry-point address — it propagates the PE optional-header's `AddressOfEntryPoint` field: `ep = x_Exe->m_OptionalHeader.m_entry + m_Header.dwPeBaseAddr`. Therefore "what code runs before `WinMainCRTStartup`" reduces to "what symbol the PE entry-point address points at" — a per-XBE link-flag concern, NOT a cxbe-source-edit concern. The `-entry:` flag in nxdk-link / lld-link does exactly that. Smallest meaningful implementation of candidate B = a single thunk function defined locally in witness-only + a one-line NXDK_LDFLAGS addition. ZERO nxdk source edits required; ZERO `lib/*` edits required (the existing cycle-29 self-witness shim from `lib/xbed_self_witness.c` is reused verbatim). This is functionally equivalent to (and strictly smaller blast radius than) the cxbe-modification framing the cycle-42A closure proposed.

**Codex 5-round validation** (mode=`changes`): R1 MAJOR ISSUES (HIGH "0xA6+count=0 row overclaims calling-context ELIMINATED — vsnprintf-pre-libc fault could yield same observable without allocator ever attempted" + MED "0xA4+count=0 row describes logically-impossible flow" + LOW cosmetic "tail-call" wording; all ADOPTED via documentation rewrite — HIGH row reframed as INCONCLUSIVE, MED row removed with explanatory paragraph) → R2 MED ("phys-range and alignment guards sit POST-allocation, not pre-allocation — the failure-mode list mis-attributes them"; ADOPTED via (a)/(b)/(c) failure-mode taxonomy: (a) pre-allocator fault, (b) allocator-then-post-guard rejection, (c) true allocator rejection) → R3 HIGH ("success row over-interprets reserved1 — both 'pre-WinMain succeeded as first allocator' and 'pre-WinMain failed but later fire succeeded' can yield reserved1>=1"; ADOPTED via reserved1-keyed multi-row table) → R4 NOT GREEN (HIGH "reserved1+execution-died ambiguity remains because a crash between fires can produce the same reserved1 for different first-successful-allocator indices" + LOW "cycle-23 XCTR fires mis-attributed as WTNS-page consumers"; HIGH ADOPTED via JOINT (reserved0_last_stage, reserved1) 14-row table defining M = i-index of last successful fire from stage byte (0xA6→1, 0xA4→2, 0xA5→3, 0xA1→4, 0xA3→5) and k = M - reserved1 + 1 = first-successful-allocator index; LOW ADOPTED via clarification that XCTR fires use the separate `xbed_a4_witness_fire` shim on the agent's XCTR buffer, not the WTNS page) → R5 GREEN — explicit "Deploy-ready" with no new findings, no open questions, no out-of-scope issues. All hard findings adopted via in-source documentation rewrites; ZERO C-source body changes between R1 and R5 (the function body is bit-identical from the first successful build to the R5-GREEN state — all 5 rounds of churn were comment/documentation refinement). Codex marker at `.claude/state/codex-validate-last-run` refreshed.

**Build verification.** Clean nxdk rebuild succeeds with custom entry; lld emits no errors (only the standard `.edata=.rdata: already merged into .edataxb` warning seen in every nxdk build). PE-level verification: `llvm-readobj --file-headers main.exe` reports `AddressOfEntryPoint=0x3510` (ImageBase 0x10000, virtual VA 0x13510); `llvm-objdump -d main.exe --start-address=0x13510` confirms the expected sequence (`s_witness_only_42b_eeprom_attempted` sticky check → `HalWriteSMBusValue` import-thunk call with args 0xA8/0xFF/0x0/0xA6 → `xbed_self_witness_fire(6)` direct call → `WinMainCRTStartup` direct call) with the correct register values. Final cycle-42B witness-only deployed-build SHA = `3cc670fb4df5871811d94bcc8339e5126ab0e61a78e44802ba9e655d742442a1` (155 648 B; same XBE page boundary as cycles 31..42A; source bit-identical to Codex-R5-GREEN-confirmed state). Oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`.

**Real-Xbox deployment DEFERRED.** The bounded slice covers implementation+build+Codex. Cycle-42B real-Xbox deployment is the next bounded slice (Hermes's call). Runbook delta vs cycle 42A documented in `benchmark-runs/cycle42b-prewinmaincrt-20260524T115521Z/SUMMARY.md`: expected post-run EEPROM byte is now `0xA6` (not `0xA4`); `witness.scan-self` interpretation requires BOTH count + reserved0_last_stage + reserved1 (cycle 42A only needed count + reserved0); the cleanest "calling-context CONFIRMED" signal is (stage=0xA3, reserved1=5, count>=1) ⇒ k=1 ⇒ pre-WinMain was the first successful allocator AND all five WTNS-shim fires reached. Three plausible outcome rows with next-step actions: (a) calling-context CONFIRMED ⇒ cycle 43 single-axis variations of the pre-WinMain context (no-cookie / no-TLS / no-libc / no-thread / different-stack-frame) to isolate which property is operative; (b) gradient signal (k>1) ⇒ cycle 43 investigates why pre-WinMain context fails specifically; (c) INCONCLUSIVE (count=0, EEPROM=0xA6) ⇒ cycle 42C+ needs stage-6-specific shim bypass behavior OR additional EEPROM marker bytes at each decision point; (d) regression (count=0, EEPROM=0x00) ⇒ re-validate build artifact.

**Preserved invariants.** Cycle-23 lockstep contract (`lib/xbed_a4_witness.{c,h}`) UNCHANGED. Cycle-29 self-witness shim (`lib/xbed_self_witness.{c,h}`) UNCHANGED — cycle-42A 2-page allocation + cycle-41c phys-range guards + cycle-41d alignment guard + cycle-39 EEPROM sticky-flag gate ALL preserved in their cycle-42A state. Cycle-35 .CRT$XXC + .CRT$XCU slot fires in `witness-only/main.c` UNCHANGED; they now run as 2nd/3rd calls to the idempotent shim instead of 1st/2nd as in cycle 42A. Cycle-31 visual breadcrumb path UNCHANGED. Cycle-39 EEPROM scratchpad mechanism UNCHANGED in the shim; cycle-42B adds a SEPARATE defensive write to the same byte with the same encoding (0xA0 | stage = 0xA6) so the post-run byte is deterministic regardless of vsnprintf behavior in pre-libc context. oracle-agent (`oracle-agent` tree) UNCHANGED — existing `witness.scan-self` + `eeprom.scratch.read` verbs report the cycle-42B signal shape (count, reserved0, reserved1) without modification. All eight default-on Apple Silicon flags UNCHANGED (cycle 42B is XBE-side instrumentation only, no host xemu source touched, project rule #11 not in play).

Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail. Pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged. 25+ pre-existing untracked `.hermes_*` files at repo root preserved un-staged. Cycle-42B witness-only XBE deployed-build SHA = `3cc670fb4df5871811d94bcc8339e5126ab0e61a78e44802ba9e655d742442a1` (155 648 B). Evidence-only files at `benchmark-runs/cycle42b-prewinmaincrt-20260524T115521Z/{SUMMARY.md, codex-r1..r5-prompt.md, codex-r1..r5-output.md}` are gitignored per project convention.

## 2026-05-24 (cycle 42A multi-page cycle-29 self-witness redesign — CLOSED on `apple-silicon-performance`): OUTCOME G0(c) PERSISTS under 0x2000 multi-page allocation; STRONG evidence AGAINST cycle-22 branch (c) being the failing axis at the smallest multi-page step; cycle-22 candidate set within in-XBE/lib-only scope EXHAUSTED; cycle 42 candidate B (pre-`_start` callback via `nxdk/tools/cxbe/`) is the necessary follow-up

**Decision.** Cycle 42A executes the cycle-41e closure's recommended candidate A: bump the cycle-29 self-witness allocator request from `0x1000u` (1 page, cycle 41e) to `0x2000u` (2 pages, 8 KiB) in `lib/xbed_self_witness.c`; bump `MmPersistContiguousMemory(p, 0x2000u, TRUE)` and the page-wipe loop bound to match. WTNS magic + version + reserved0 + reserved1 header still lives ONLY at offset 0 of the FIRST page of the multi-page allocation; the second page is zero-filled by the page-wipe loop so it fails the consumer's WTNS magic predicate at the next 0x1000-stride read. Cycle-29 consumer at `oracle-agent/commands.c::cmd_witness_scan_self` is comment-only updated (NO logic change) because the existing 0x1000-stride scan finds the WTNS magic on the first page (page-aligned per the cycle-41d guard); `count` still grows by exactly 1 per cycle-42A allocation. Cycle-41c symmetric phys-range guards + cycle-41d page-alignment guard PRESERVED unchanged — they check the FIRST page of the allocation, which is sufficient because the WTNS magic + header live exclusively on the first page; guard log strings relabeled `cycle-41e` → `cycle-42A` and updated to note the first-page-only nature of the check. Cycle-29 first-call branch / cycle-39 EEPROM-write breadcrumb + sticky `s_eeprom_scratch_attempted` gate / `wbinvd` stamp / `phys | 0x80000000` cached-mirror alias for the readback path: ALL UNCHANGED. Cycle-23 lockstep + cycle-29 self-witness shim + cycle-31 paint sequence + cycle-35 `.CRT$X*` slot mechanism + cycle-39 EEPROM-scratchpad path + cycle-41a..41e historical comment blocks: ALL PRESERVED. Header `xbed_self_witness.h` Safety-notes block gains a new "Cycle-42A" subsection (authoritative current-behavior description with discriminator semantics + Honest-framing carried over from cycle 41e); function-contract docstring rewritten with explicit "Current live behavior (CYCLE-42A multi-page redesign; supersedes ...)" paragraph + history-fence pointer to the Cycle-42A subsection; cycle-29 option-(c) overview paragraph updated to distinguish cycle-29 original design (1-page, `-Ex`) from cycle-42A current live behavior (2-page, non-`-Ex`). Codex 4 rounds (mode=`changes`): R1 MINOR (MED header-staleness ADOPTED via function-contract rewrite + cycle-29 overview history-fence; LOW consumer count>=2 leak shape ADOPTED via consumer-comment rewrite) → R2 MINOR (1 new MED "logically-impossible 2-page-free-when-no-1-page-hole example" ADOPTED in both `.c` and `.h` via replacement with internally-distinct-code-path rationale: size-bucketed free lists / separate pool arenas / distinct minimum-size policies for contiguous-memory allocations from a pre-`main()` calling context) → R3 NOT-GREEN (1 new MED "mapped_pages_seen is the kseg0 survey counter, not a per-allocation counter" ADOPTED via rewrite clarifying `count` (grows by 1 per allocation) vs `mapped_pages_seen` (survey counter; ~419 stable across runs on retail); ALSO confirms cycles 41a..41e all reported 419) → R4 GREEN — explicit "Deploy-ready". All hard findings adopted. Codex marker refreshed.

**Why the size choice was 0x2000 and not 0x4000 / 0x8000.** 0x2000 is the smallest multi-page size that genuinely tests branch (c) "`size=0x1000`-specific interaction". Larger sizes would (i) increase contiguous-RAM pressure on the real-Xbox kernel allocator and risk a NEW failure mode (allocator-pool exhaustion) confounding the size-axis signal, and (ii) widen blast radius without adding information. 0x2000 is the minimal increment that genuinely varies the size axis while keeping every other cycle-41e invariant intact.

**Real-Xbox outcome.** Cycle-40-shape runbook, 17 logged steps. Pre-baseline scans MET (cycle-41e leftovers); reboot at 2026-05-24T10:39:14Z → dashboard FTP back at t+35s; FTP-upload witness-only with `--overwrite` (uploaded=1); ensure-agent (v0.5 re-launched from cycle 41e); `unsafe.enable` + `eeprom.scratch.reset` → byte=0x00 baseline confirmed; `witness.scan-self count=0` + `witness.scan D-cycle-27` preconditions MET; `runxbe path=E:\Apps\witness-only\default.xbe` at 2026-05-24T10:40:34Z; dashboard FTP back at t+36s (consistent with cycle-36 t+38s graceful HalReturnToFirmware shape; slower than cycles 41a..41e t+6..24s but recovery timing NOT load-bearing). Final evidence: EEPROM byte=`0xA4`, `witness.scan-self count=0 mapped_pages_seen=419`, `witness.scan = D-cycle-27` (phys=0x03eb3000). Full `eeprom` hex dump cross-check confirmed last byte = `A4`. **OUTCOME = G0(c) PERSISTS under 0x2000 multi-page allocation** per the cycle-40 G-row table. `MmAllocateContiguousMemory(0x2000u)` STILL did NOT yield a usable allocation on this real-Xbox kernel from the cycle-29 `.CRT$XXC` slot calling context — identical observable shape to cycles 40 + 41a..41e single-page.

**Hypothesis-state update.** Per the cycle-42A discriminator semantics: EEPROM byte=0xA4 + count=0 at 0x2000u → STRONG evidence AGAINST size being the operative axis at the smallest multi-page step. Combined elimination ledger across cycles 40 + 41a + 41b (cache-policy under `-Ex`) + 41c (matched-tuple address range under `-Ex`) + 41d (alignment-drop under `-Ex`) + 41e (non-`-Ex` ABI at 1 page) + 42A (multi-page at 2 pages):

1. **Cache-policy** (bare RW / NC / WC under `-Ex`): EXHAUSTED.
2. **Address-range matched-tuple** under `-Ex`: ELIMINATED.
3. **Page-alignment requirement** under `-Ex`: ELIMINATED.
4. **`-Ex` validation logic alone** as failing constraint: STRONG evidence AGAINST (cycle 41e).
5. **Size-as-validation-axis at smallest multi-page step**: STRONG evidence AGAINST (cycle 42A).

The cycle-22 candidate set within the in-XBE / lib-only oracle workflow scope is now EXHAUSTED.

Caveats (per cycle-42A Honest-framing paragraph in source): cycle-42A is NOT a pure single-axis discriminator. Bumping `size` from `0x1000` to `0x2000` may interact with the kernel's pool-search policy through internal code-path divergence (size-bucketed free lists / separate pool arenas / distinct minimum-size policies for contiguous-memory allocations from a pre-`main()` calling context) rather than the size-as-validation-axis being what was tested. A formal closure of branch (c) would require sweeping size across multiple multi-page steps OR cross-validation with candidate B.

**Recommended cycle 42 candidate B (binding contingent path for the next cycle).** Custom XBE-header callback before `_start` via `nxdk/tools/cxbe/` modifications — separates the calling-context axis from the size axis; strictest pre-CRT context distinguishes "calling-context-specific failure inside the cycle-29 `.CRT$XXC` slot" from "shared-upstream failure independent of calling context". Larger blast radius than candidate A (requires modifying nxdk's XBE-header generator), but the calling-context axis is now the only remaining live cycle-22 candidate that can be tested inside the in-XBE oracle workflow. If candidate B also yields G0(c), the cycle-22 hypothesis has been varied across every axis the in-XBE / lib-only workflow exposes and the next step would necessarily move outside that scope (e.g. host-xemu instrumentation of the guest allocator path; high scope; violates the cycle-23 lockstep contract).

Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail. Pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged. 24+ pre-existing untracked `.hermes_*` files at repo root preserved un-staged. New cycle-42A witness-only XBE deployed-build SHA = `698e6fefeff916daf287cdff02f8bda72167687025aa57cd0d51568e1a1d054f` (155 648 B; oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`). Evidence-only files at `benchmark-runs/cycle42a-multipage-20260524T103826Z/{SUMMARY.md, codex-output-r1..r4.md, codex-prompt-r1..r4.md, 00..17-*.{txt,json}}` are gitignored per project convention.

## 2026-05-24 (cycle 41e non-`-Ex` fallback variation — CLOSED on `apple-silicon-performance`): OUTCOME G0(c) PERSISTS under non-`-Ex` fallback; STRONG evidence AGAINST cycle-22 branch (b) being sole failing constraint; cycle-41 SCOPE EXHAUSTED; cycle 42 must choose between multi-page cycle-29 redesign OR custom XBE-header callback before `_start`

**Decision.** Cycle 41e executes the cycle-41d closure's binding contingent path: replace the cycles-29..41d `MmAllocateContiguousMemoryEx(0x1000u, 0x00000000u, 0x7FFFFFFFu, 0u, PAGE_READWRITE | PAGE_WRITECOMBINE)` 5-arg call with the non-`-Ex` `MmAllocateContiguousMemory(0x1000u)` 1-arg call in `lib/xbed_self_witness.c`. The 1-arg form is the actual nxdk API (`nxdk/lib/xboxkrnl/xboxkrnl.h:3473-3476`; export `MmAllocateContiguousMemory@4`; in-tree precedent `nxdk/lib/hal/xbox.c:34` + `:80`). The cycle-41d closure docs described the substitution as a 2-arg `MmAllocateContiguousMemory(size, protect)` call — that signature is invalid against the nxdk header (would not compile). The 1-arg call is correct and is implemented; the prompt-vs-header reconciliation is documented in-source. Cycle-29 first-call branch / EEPROM-write breadcrumb (comment + log strings degenericized to drop the historical `-Ex` name) / sticky `s_eeprom_scratch_attempted` gate / `MmPersistContiguousMemory` / `wbinvd` stamp / `phys | 0x80000000` cached-mirror alias for the readback path: ALL UNCHANGED. Cycle-23 lockstep + cycle-29 self-witness + cycle-31 paint + cycle-35 `.CRT$X*` slot + cycle-39 EEPROM-scratchpad (sticky-flag-Codex-R2-P1 gate) + cycle-41a comment tightening + cycle-41b comment block + cycle-41c symmetric phys-range guards + cycle-41d page-alignment guard: ALL PRESERVED. The cycle-41c symmetric phys-range guards + cycle-41d page-alignment guard remain LOAD-BEARING in cycle-41e because the non-`-Ex` ABI gives the caller no control over Protect / placement / Alignment — the kernel picks all three, and the cycle-29 consumer scan window (`[0x80010000, 0x84000000]`, 0x1000 stride) is unchanged. Guard log strings relabeled `cycle-41d` → `cycle-41e`. Header `xbed_self_witness.h` Safety-notes block rewritten to document the cycle-41e non-`-Ex` divergence, in-tree precedent, narrowed cycle-22 candidate set, post-cycle-41e fallback paths. Codex 4 rounds (mode=`changes`): R1 MAJOR (P1 doc-drift DEFERRED to this closure commit, P2 "ELIMINATES branch (b)" overstated ADOPTED via Honest-framing, P3 line-277 inconsistency ADOPTED) → R2 MAJOR (1 new sign-flip ADOPTED) → R3 MINOR (3 LOW: stale `-Ex` wording ADOPTED via degenericization, `.def:172` REBUTTED with grep evidence — `:172` is file line, `@ 165` is symbol ordinal; "historically PAGE_READWRITE" unsupported ADOPTED via softening) with explicit "Deploy-readiness: green" → R4 MINOR (1 residual LOW; pre-existing historical references; Codex "Comment-only, not a deployment blocker"; DEFERRED). All hard findings adopted. Codex marker refreshed.

**OUTCOME: G0(c) PERSISTS** per the cycle-40 G-row table: post-chainload EEPROM byte at 0xFF = `0xA4` (tag=0xA, stage_nib=0x4) + `witness.scan-self count=0 mapped_pages_seen=419` + `witness.scan = count=1 phys=0x03eb3000 reserved0=0 reserved1=0` (D-cycle-27 shape) — unchanged from cycles 40 + 41a + 41b + 41c + 41d. Full `eeprom` hex dump cross-check confirmed last byte = `A4`. `MmAllocateContiguousMemory(0x1000u)` STILL did NOT yield a usable allocation on this real-Xbox kernel from the cycle-29 `.CRT$XXC` slot calling context. Dashboard FTP recovery at t+16s post-chainload (consistent watchdog reset shape; not load-bearing for G-row classification). New cycle-41e witness-only XBE deployed-build SHA = `5c9fad12d592117392761f503919fff85c53abe3b26773588b5c1365d2e765e6` (155 648 B; source bit-identical to codex-R4 state); oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`.

**Cycle-22 hypothesis state.** Cycle 41e provides STRONG evidence AGAINST branch (b) "the `-Ex` variant itself is the failing constraint" being the SOLE failing constraint — both `-Ex` (cycles 40 + 41a + 41b + 41c + 41d) AND non-`-Ex` (cycle 41e) fail identically against this calling context. Cycle 41e does NOT formally eliminate branch (b) on its own (the non-`-Ex` defaults that the kernel picks for Protect / placement / Alignment are not measured on this hardware; a shared upstream failure mode common to both entry points remains conceivable), but it does eliminate the simpler "`-Ex` validation logic specifically rejects this tuple" hypothesis. **Cycle-41 scope is now exhausted.** Combined elimination ledger across cycles 40+41a+41b (cache-policy under `-Ex`) + 41c (address-range under `-Ex`) + 41d (alignment-requirement under `-Ex`) + 41e (non-`-Ex` fallback) means the remaining live cycle-22 candidate is branch (c) **`size=0x1000`-specific interaction** — OUT OF cycle-41 scope (requires redesigning the cycle-29 self-witness as multi-page; changes the WTNS layout contract). Reproducibility shape continues: `phys=0x03eb3000` deterministic kernel-pool reuse + `mapped_pages_seen=419` across 25+ consecutive observations (cycles 26..41e).

**Why this matters.** Cycle 41 was scoped as a focused allocator-acceptance sweep: vary one axis at a time (cache-policy → address-range → alignment → entry-point), preserve cycle-29 single-page WTNS layout, classify each variation against the cycle-40 G-row table. Five bounded variations have produced G0(c) PERSISTS for every cache-policy bit, every address-range degree of freedom, every alignment choice the API accepts, and both allocator entry points. That convergence is itself the most discriminating cycle-22 evidence to date: the failing constraint is not in any axis cycle 41 could vary inside the WTNS layout contract; it is in something cycle 41 could not vary — the single-page `size` request from the cycle-29 `.CRT$XXC` slot calling context. Cycle 42 is the natural follow-up: either change the size axis (multi-page) or change the calling context (custom XBE-header callback before `_start`).

**Recommended cycle 42 (Hermes's call).** Two out-of-cycle-41-scope candidates:

- **(A) Multi-page cycle-29 self-witness redesign** — tests branch (c) directly. Allocate ≥2 pages (likely 0x2000; investigate whether the kernel coalesces page-table entries differently at multi-page granule); update the consumer reader at `oracle-agent/commands.c::cmd_witness_scan_self` to walk at the new stride; may need WTNS layout magic/version bump for the multi-page header. Tests both allocator entry points (`-Ex` AND non-`-Ex` could be tried in sequence to confirm). Smaller blast radius than (B) — stays inside `lib/` + `oracle-agent/` source tree.
- **(B) Custom XBE-header callback before `_start`** — separates the calling-context axis from the size axis. Requires changes to `nxdk/tools/cxbe/`'s XBE-image-header entry-point generator to install a callback that runs BEFORE `_start` / `__security_init_cookie` / `_PDCLIB_xbox_libc_init`. The callback would attempt the same 1-page `MmAllocateContiguousMemory(0x1000u)` from the strictest pre-CRT context. If the allocation succeeds from the XBE-header-callback context but fails from the `.CRT$XXC` slot context, the failing constraint is a calling-context interaction (rules engaged later in the CRT init sequence). If it ALSO fails from the XBE-header context, the failing constraint is size-independent of calling context — strongest evidence for branch (c). Larger blast radius (touches `nxdk/tools/cxbe/`).

Recommended ordering: candidate A first (smaller blast radius; faster iteration; bounded change to two source trees); candidate B is the necessary follow-up if A also yields G0(c) PERSISTS or if A's redesign produces ambiguous evidence about whether size is the operative variable.

**What's preserved (NO changes).** ZERO host xemu source touched. ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact). ZERO `lib/lib.mk` touched. ZERO `oracle-agent/*` touched (cycle-39 v0.5 verbs intact; cycle-41e non-`-Ex` swap is producer-side only — consumer scan window unchanged). ZERO `xbed_runtime.{c,h}` touched. ZERO `witness-only/main.c` / Makefile / manifest.json touched. ZERO `nxdk/` source touched (read-only inspection of `xboxkrnl.h` + `xboxkrnl.exe.def` + `hal/xbox.c` for the non-`-Ex` precedent). ZERO `tools/xemu-capture/` source touched. ZERO `composite-record.sh` / `composite-preflight.sh` source touched. NO net change to any flag default; M15 unchanged. NO PushNotification (run-only outcome; single bounded variation that did not flip the gate). Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` + 2 `scripts/apple-silicon/xbe-tests/lib/*.inl` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision guardrail; pre-existing untracked `scripts/apple-silicon/composite_preflight.py` + 23+ pre-existing untracked `.hermes_*` files at repo root preserved un-staged per the same pattern.

---

## 2026-05-24 (cycle 41d alignment-drop variation — CLOSED on `apple-silicon-performance`): OUTCOME G0(c) PERSISTS under alignment-drop; cycle-22 narrowed to {the `-Ex` variant itself, `size=0x1000`-specific interaction}; cycle 41e explores non-`-Ex` fallback

**Decision.** Cycle 41d executes the cycle-41c closure's binding contingent path: change the witness-only `MmAllocateContiguousMemoryEx` alignment argument in `lib/xbed_self_witness.c:214` from `0x1000u` (cycles 29..41c) to `0u` (let the real-Xbox kernel pick natural page-granular alignment for contiguous memory). All other cycle-41c arguments — matched-tuple address range (`lowest=0x00000000u, highest=0x7FFFFFFFu`) + `Protect = PAGE_READWRITE | PAGE_WRITECOMBINE` (cycle 41b) — UNCHANGED. Plus Codex-round-1 P1-adopted page-alignment guard at `xbed_self_witness.c:317-346` rejecting any returned `phys` not 0x1000-aligned (cycle-29 consumer scans at fixed 4 KiB stride; with `Alignment=0u` the kernel could in principle return a sub-page-aligned phys that the consumer cannot see). Plus Codex-round-1 P3 low-adopted cycle-41c→cycle-41d log-string relabel at `xbed_self_witness.c:296` + `:307`. All cycle-23 lockstep / cycle-29 self-witness / cycle-31 paint / cycle-35 `.CRT$X*` slot / cycle-39 EEPROM-scratchpad + sticky-flag-Codex-round-2 P1 gate / cycle-41a comment tightening / cycle-41b comment block / cycle-41c symmetric phys-range guards: PRESERVED.
...
**Rationale.** Cycle-41c closure pre-recorded the cycle-41d candidate explicitly: "alignment-drop variation: change `xbed_self_witness.c` `alignment` argument from `0x1000u` to `0u` (let kernel pick alignment); bounded single-literal change. If cycle 41d still fails G0(c), only non-`-Ex` fallback to plain `MmAllocateContiguousMemory(0x1000)` (cycle 41e) remains in cycle-41 scope." Cycle 41d is the bounded execution of that candidate. In-tree precedent for `Alignment=0u` against `MmAllocateContiguousMemoryEx` exists at `nxdk/lib/pbkit/pbkit.c:2297` (pbkit's `pb_DmaBuffer*` + `pb_Head` allocations), `nxdk/samples/{triangle,mesh,xaudio}/main.c` (every drawing sample), and `xbe-tests/flat-tri-depth/main.c:77`. The nxdk header at `xboxkrnl.h:3464` is a bare prototype with no `Alignment=0` doc text, so the in-tree calls demonstrate only that the API accepts `0u`, not what alignment the kernel returns — the new page-alignment guard closes that interpretation gap so a `count=0` observation remains well-defined.

**Cycle-22 hypothesis state after cycle 41d.**
- Cache-policy variations EXHAUSTED (cycle 40 bare RW + cycle 41a NC + cycle 41b WC all G0(c)).
- Address-range matched-tuple sub-hypothesis ELIMINATED (cycle 41c: nxdk-fb-matched tuple modulo size also G0(c)).
- **Page-alignment requirement ELIMINATED (cycle 41d: `Alignment=0u` also G0(c)).**
- Remaining candidates in cycle-41 scope:
  ...
  2. **`size=0x1000`-specific interaction** — only `size` still differs from nxdk's framebuffer allocator (their `screenSize` is multi-page; ours is single-page); out-of-cycle-41-scope (would require redesigning the cycle-29 self-witness as multi-page; changes the WTNS layout contract).

If cycle 41e also fails G0(c), only fundamentally different approaches remain: custom XBE-header callback before `_start` (high scope; requires `nxdk/tools/cxbe/` changes), or redesign of cycle-29 self-witness as multi-page.

**Codex validation 3 rounds** (mode=`changes`):
- R1 MAJOR ISSUES: P1 high "alignment-drop blind spot on consumer 4 KiB stride" ADOPTED via `(phys & 0xFFFu) != 0u` guard at `xbed_self_witness.c:317-346`; P3 low "stale cycle-41c log strings" ADOPTED via cycle-41d relabel.
- R2 MINOR ISSUES: R1.P1 + R1.P3 confirmed addressed; one low comment-precision finding (already satisfied by in-source soft framing — "in-tree usage pattern", "we cannot point to a documented ... guarantee", "API accepts `0u`, not what alignment the kernel returns").
- R3 GREEN: explicit "This slice is deploy-ready for the real-Xbox run."

Codex marker at `.claude/state/codex-validate-last-run` refreshed.

**Build.** Cycle-41d witness-only deployed-build SHA = `c49ca0ad2a3f968ecb0ea02bf16a7117da614b912ff98feb58ed925ee387f589` (155 648 B; SHA varies between rebuilds of identical source because XBE/COFF format embeds build timestamps; source bit-identical to codex-validated GREEN state). Oracle-agent unchanged from cycle-39 v0.5 SHA `d419b452…`.

**Real-Xbox run sequence.** 18 logged steps under `benchmark-runs/cycle41d-alignment-20260524T054838Z/`. Reboot 1 at 06:00:02Z → dashboard FTP back at t+20s → FTP-upload witness-only `--overwrite` → ensure-agent → EEPROM baseline armed → preconditions MET → `runxbe path=E:\Apps\witness-only\default.xbe` at 06:03:09Z → dashboard FTP back at t+19s → ensure-agent → final scans = G0(c) PERSISTS → full eeprom hex dump cross-check = last byte `A4`. Composite capture SKIPPED with cycle-34+36+41a+41b+41c silent-stall rationale.

**Reproducibility wins.** `phys=0x03eb3000` deterministic kernel-pool reuse REPRODUCED across baseline + post-deployment + final readbacks (24+ consecutive observations across cycles 26..41d). `mapped_pages_seen=419` REPRODUCED at every readback. EEPROM non-volatility across the pre-chainload reboot 1 CONFIRMED.

**Scope discipline.** ZERO host xemu source touched; ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact); ZERO `lib/lib.mk` touched; ZERO `oracle-agent/*` touched (cycle-39 v0.5 verbs intact; cycle-41d alignment-drop is producer-side only); ZERO `xbed_runtime.{c,h}` touched; ZERO `witness-only/main.c` / Makefile / manifest.json touched; ZERO `nxdk/` source touched (read-only inspection of `pbkit.c`, `hal/video.c`, `xboxkrnl.h`, samples for the precedent search); ZERO `tools/xemu-capture/` source touched; ZERO `composite-record.sh` / `composite-preflight.sh` source touched. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` PRESERVED unstaged per the rolling cycle-34+ Hermes-supervision pattern; pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged; 22+ pre-existing untracked `.hermes_*` files at repo root preserved unstaged.

**M15 overall still NOT MET** pending §H.6 default-on shape (now blocked on cycle-41e+ non-`-Ex` fallback work + downstream cycles), §G.5, RT-as-texture; `XEMU_DIAG_PGRAPH_STATUS_DRAIN` default-on / long-term-fix decision REMAINS DEFERRED.

## 2026-05-24 (cycle 41c combined address-range variation — CLOSED on `apple-silicon-performance`): OUTCOME G0(c) PERSISTS under matched-tuple address range; cycle-22 narrowed to {alignment, `-Ex` variant, or `size`-specific interaction}; cycle 41d explores alignment-drop

**Decision.** Cycle 41c executes the cycle-41b closure's binding contingent path: change the witness-only allocation tuple to match nxdk's framebuffer allocator (`nxdk/lib/hal/video.c:363-367`) BYTE-FOR-BYTE modulo `size`. Two literal changes at `lib/xbed_self_witness.c:155-156`: `lowest=0x00010000u` → `0x00000000u` and `highest=0x03FFFFFFu` → `0x7FFFFFFFu`. Alignment `0x1000u` + `Protect = PAGE_READWRITE | PAGE_WRITECOMBINE` UNCHANGED from cycle-41b. Plus Codex-round-1+round-2 P1-adopted symmetric phys-range guards (`phys < 0x00010000u` AND `phys >= 0x04000000u`) at `xbed_self_witness.c:254-273` that reject any returned phys outside the cycle-29 consumer's scan window `[0x00010000, 0x04000000)` so a `witness.scan-self count=0` observation cannot ambiguously mean "kernel returned an out-of-reader-window phys". Plus comment block + header doc updates documenting the cycle-41c divergence, the matched-tuple precedent, and the symmetric guards. All cycle-23 lockstep / cycle-29 self-witness / cycle-31 paint / cycle-35 `.CRT$X*` slot / cycle-39 EEPROM-scratchpad path / cycle-39 sticky-flag-Codex-round-2 P1 gate / cycle-41a comment tightening / cycle-41b comment block: PRESERVED.

**Outcome = G0(c) PERSISTS under matched-tuple address range.** Post-chainload signals identical to cycles 40 + 41a + 41b: EEPROM byte at 0xFF = `0xA4` + `witness.scan-self count=0` + `witness.scan = D-cycle-27` shape. `MmAllocateContiguousMemoryEx(0x1000, 0x00000000, 0x7FFFFFFF, 0x1000, PAGE_READWRITE | PAGE_WRITECOMBINE)` — the EXACT nxdk framebuffer allocator tuple modulo `size` — STILL did NOT yield a usable allocation on this real-Xbox kernel. Dashboard FTP recovery at t+24s post-chainload — IDENTICAL to cycle 41b (+18s vs cycle-40 baseline; recovery timing NOT load-bearing for G-row classification).

**Rationale.** Cycle-41b closure pre-recorded the cycle-41c candidate explicitly: "combine the lowest-scope remaining candidates into ONE variation that matches the nxdk framebuffer-allocator address range exactly: `lowest=0x00000000, highest=0x7FFFFFFF, alignment=0x1000, Protect = PAGE_READWRITE | PAGE_WRITECOMBINE`. This is the closest possible match to a known-good nxdk-side call (`nxdk/lib/hal/video.c:363-367` parameters exactly modulo `size`) and represents the lowest-information-cost test of 'is the address range OR the protect bit the failing constraint.'" Cycle 41c is the bounded execution of that candidate. The framebuffer allocator runs successfully on every nxdk-built XBE that draws anything on this exact kernel; if the kernel rejects the cycle-41c tuple but accepts nxdk's, the failing constraint cannot be the address range alone — narrowing the cycle-22 search to {alignment, `-Ex` variant, `size`-specific interaction}.

**What cycle-41c discriminates.** ELIMINATES the cycle-22 sub-hypothesis "kernel demands a specific non-cycle-29-tuple address range" (within retail-Xbox 64 MiB physical RAM scope — the only addresses the kernel can return on this hardware). Combined with cycles 40 + 41a + 41b (cache-policy exhaustion), the failing constraint is now narrowed to: (i) alignment requirement `0x1000` (cycle 41d will test `alignment=0`), (ii) the `-Ex` variant itself (cycle 41e will fall back to plain `MmAllocateContiguousMemory`), or (iii) a `size=0x1000`-specific interaction (only `size` still differs from nxdk's known-good framebuffer allocator call). The size axis is out-of-cycle-41-scope (would require redesigning the cycle-29 self-witness as multi-page; would expand scope significantly).

**Codex review (3 rounds).**
- **Round 1: MAJOR ISSUES.** P1 high "cycle-29 consumer at `oracle-agent/commands.c:726` scans only `0x80010000..0x84000000` (64 MiB kseg0 window); cycle-41c's `highest=0x7FFFFFFF` theoretically allows kernel to return phys above 64 MiB invisible to consumer, turning `count=0` into false-negative G0(c)." ADOPTED via 5-LOC upper-bound guard rejecting `phys >= 0x04000000`. P2 medium "cycle-41c elimination claim overstates" ADOPTED via retail-64MiB-scope qualification. P2 medium "doc-rule-#4 mid-slice drift" DEFLECTED (handoff.md / claude-status.md showed cycle 41b mid-slice because closure commit had not yet landed; transient state, not real drift; closure commit syncs). P3s confirming flag-tuple match accuracy.
- **Round 2: MAJOR ISSUES.** P1 high "symmetric low-phys blind spot — cycle-41c's `lowest=0x00000000` could let kernel return phys < 0x00010000 (cycle-29 reader's lower bound); equally invisible to consumer." ADOPTED via 5-LOC symmetric lower-bound guard rejecting `phys < 0x00010000`. P2 medium "elimination claim still too strong" ADOPTED via further-narrowed wording: cycle 41c eliminates only the "kernel demands specific non-cycle-29-tuple address range" sub-hypothesis. P3 no new hazards introduced by guards themselves.
- **Round 3: MINOR ISSUES.** P2 medium "count=0 comment overstates as unambiguously 'allocation failed'" ADOPTED via wording softening — count=0 on cycle-41c is STRONGLY suggestive of allocation-failure but not unambiguous because either guard firing also yields count=0. Codex explicit: "Nothing else looks load-bearing for the cycle-41c real-Xbox deploy."

Codex marker at `.claude/state/codex-validate-last-run` refreshed (fingerprint `cc437e2b`; verdict MINOR ISSUES with all R1+R2 P1 findings adopted and R3 P2 comment finding adopted).

**What this cycle does NOT do.** No host xemu source edits. No `lib/xbed_a4_witness.{c,h}` edits (cycle-23 lockstep contract intact). No `lib/lib.mk` edits. No `oracle-agent/*` edits (cycle-39 v0.5 verbs intact; the symmetric phys-range guards are PRODUCER-side defense; no consumer-side change). No `xbed_runtime.{c,h}` edits. No `witness-only/main.c` / Makefile / manifest.json edits. No `nxdk/` source edits (`../nxdk/lib/hal/video.c` was read-only inspected only). No `tools/xemu-capture/` edits. No `composite-record.sh` / `composite-preflight.sh` edits. No scope-expansion to additional allocation variations in the same session (cycle-41d alignment-drop is deferred to its own bounded slice).

**Recommended cycle 41d (binding contingent path for the next cycle).** Alignment-drop variation: change `xbed_self_witness.c` `alignment` argument from `0x1000u` to `0u` (let the kernel pick alignment). Bounded single-literal change. If cycle 41d still fails G0(c), only non-`-Ex` fallback to plain `MmAllocateContiguousMemory(0x1000)` (cycle 41e) remains in cycle-41 scope; that is the last allocation-variation slice before the cycle-22 narrowing forces either a fundamentally new approach (custom XBE-header callback running before nxdk's `_start`; high scope — requires modifying `nxdk/tools/cxbe/`) or a redesign of the cycle-29 self-witness as multi-page (high scope — would change the entire WTNS layout contract). Cycle-40 G0(c) remains the cycle-41 regression gate.

## 2026-05-24 (cycle 41b `PAGE_WRITECOMBINE` allocation-flag variation — CLOSED on `apple-silicon-performance`): OUTCOME G0(c) PERSISTS under `PAGE_WRITECOMBINE`; CACHE-POLICY VARIATIONS EXHAUSTED; cycle 41c broadens to address-range

**Decision.** Cycle 41b executes the cycle-41a closure's recommended NEXT cycle-41 candidate: change `MmAllocateContiguousMemoryEx`'s `Protect` argument in `lib/xbed_self_witness.c:158` from `PAGE_READWRITE | PAGE_NOCACHE` (cycle-41a) to `PAGE_READWRITE | PAGE_WRITECOMBINE` (0x04 | 0x200 → 0x04 | 0x400). One-line code change + paired comment + header doc updates. All cycle-23 lockstep / cycle-29 self-witness / cycle-31 paint / cycle-35 `.CRT$X*` slot / cycle-39 EEPROM-scratchpad path / cycle-39 sticky-flag-Codex-round-2 P1 gate / cycle-41a Codex-adopted comment tightening: PRESERVED. **Outcome = G0(c) PERSISTS**: post-chainload EEPROM byte at 0xFF = `0xA4` + `witness.scan-self count=0` + `witness.scan = D-cycle-27` shape unchanged. `MmAllocateContiguousMemoryEx(0x1000, 0x10000, 0x3ffffff, 0x1000, PAGE_READWRITE | PAGE_WRITECOMBINE)` STILL did NOT yield a usable allocation on this real-Xbox kernel. Dashboard FTP recovery at t+24s post-chainload (cycle-41a was t+8s; +16s shift NOT dismissable as sampling variance; still faster than cycle-36 t+38s graceful; recovery timing NOT load-bearing for the G-row classification — full interpretation in cycle-41b SUMMARY).

**Rationale.** Cycle-41a closure pre-recorded the cycle-41b candidate explicitly: "PAGE_WRITECOMBINE cache-policy candidate (symmetric one-line change in `xbed_self_witness.c:138`). If WRITECOMBINE also fails identically, cache-policy variations are exhausted → cycle 41c broadens to address-range / alignment / non--Ex fallback." Cycle 41b is the bounded execution of that specific candidate. Precedent for `PAGE_READWRITE | PAGE_WRITECOMBINE` against `MmAllocateContiguousMemoryEx` exists in nxdk itself at `nxdk/lib/hal/video.c:363-367` for the framebuffer allocator — the same surface every diag XBE that paints depends on (XVideoSetMode calls MmAllocateContiguousMemoryEx with WC for the linear framebuffer). The WRITECOMBINE precedent is therefore symmetric in strength to cycle-41a's OHCI NOCACHE precedent: both are known-good nxdk-side sites that use the `-Ex` variant.

**What cycle-41b discriminates.** ELIMINATES the second cache-policy candidate "PAGE_WRITECOMBINE recovers the allocation." Combined with cycle-41a's elimination of `PAGE_NOCACHE` + cycle-40's elimination of bare `PAGE_READWRITE`, **CACHE-POLICY VARIATIONS ARE EXHAUSTED.** The cycle-22 leading hypothesis is FURTHER NARROWED: the failing constraint is NOT a cache-policy bit at all. It is one of {(i) the address-range floor `0x00010000`, (ii) the address-range ceiling `0x03FFFFFF`, (iii) the alignment requirement `0x1000`, (iv) the `-Ex` variant itself}. Cycle-41b's value is the negative result that cleanly closes the cache-policy axis with two bounded variations.

**Codex review.** Round-1 attempt v1 (`codex-prompt.md` + `codex-output.md`) got stuck in a `web_search` loop the read-only sandbox cannot service (codex tried to verify wbinvd-vs-WC Intel SDM semantics via web search; all calls failed silently; codex's output truncated mid-investigation at ~1225 lines). v3 prompt (`codex-prompt-v2.md` + `codex-output-v3.md`) replaced it with explicit "DO NOT use web search; nxdk lives at `../nxdk/...`" guidance + concise output budget. Verdict=**GREEN** (3 P3 findings, all confirmations): (P3.1) no hard-rule conflicts; (P3.2) `PAGE_NOCACHE=0x200` + `PAGE_WRITECOMBINE=0x400` are distinct non-overlapping cache-policy bits per `nxdk/lib/xboxkrnl/xboxkrnl.h:3294`; precedent claim verified at `nxdk/lib/hal/video.c:363`; (P3.3) no new coherency hazard introduced by WC vs NC since the producer-side page-zero + header-stamp + counter-increment + wbinvd all operate through the kseg0 cached-mirror alias `phys | 0x80000000` (canonicalized at `xbed_self_witness.c:165`) rather than through the kernel-returned `p` whose page-table entry carries the WC bit; the consumer scans WTNS pages via kseg0 virtual addresses in `oracle-agent/commands.c:726` — the same cycle-41a alias separation, not a new WC-specific hazard. Codex marker refreshed.

**What this cycle does NOT do.** No host xemu source edits. No `lib/xbed_a4_witness.{c,h}` edits. No `lib/lib.mk` edits. No `oracle-agent/*` edits. No `xbed_runtime.{c,h}` edits. No `witness-only/main.c` / Makefile / manifest.json edits. No `nxdk/` source edits. No tools/xemu-capture/ source edits. No composite-record.sh / composite-preflight.sh source edits. No `phys | 0xB0000000` end-to-end uncached-alias experiment (cycle-41a Codex P1-deflected suggestion; still out of scope; the readback path remains the kseg0 cached mirror). No scope-expansion to additional allocation variations in the same session (cycle-41c address-range candidate is deferred to its own bounded slice).

**Recommended cycle 41c (binding contingent path for the next cycle).** Combine the lowest-scope remaining candidates into ONE variation that matches the nxdk framebuffer-allocator address range exactly: `lowest=0x00000000, highest=0x7FFFFFFF, alignment=0x1000, Protect = PAGE_READWRITE | PAGE_WRITECOMBINE`. This is the closest possible match to a known-good nxdk-side call (`nxdk/lib/hal/video.c:363-367` parameters exactly modulo `size`) and represents the lowest-information-cost test of "is the address range OR the protect bit the failing constraint." Two-line code change at `xbed_self_witness.c:155-156`. If cycle 41c still fails G0(c), only alignment-drop (cycle 41d) and non-`-Ex` fallback (cycle 41e — `MmAllocateContiguousMemory(0x1000)` 1-arg call) remain in scope. Each variation is bounded: a ~5-LOC source change + 1 rebuild + 1 redeploy + 1 run. Cycle-40 G0(c) is the regression gate (EEPROM byte must stay 0xA4; a successful variation should advance `witness.scan-self count >= 1` with `reserved1` matching fires landed). DEFERRED beyond cycle 41e (only if all variations fail): custom XBE-header callback that runs before nxdk's `_start` — high scope (requires modifying `nxdk/tools/cxbe/`); will resurface only if cycles 41c-41e all fail in ways suggesting fundamentally unusable kernel surface.



**Decision.** Cycle 41a executes the cycle-40 closeout's recommended LOWEST-SCOPE cycle-41 candidate: change `MmAllocateContiguousMemoryEx`'s `Protect` argument in `lib/xbed_self_witness.c:138` from bare `PAGE_READWRITE` to `PAGE_READWRITE | PAGE_NOCACHE` (0x04 → 0x04 | 0x200). One-line code change + comment block updates per Codex P1+P2 findings (both ADOPTED). All cycle-23 lockstep / cycle-29 self-witness / cycle-31 paint / cycle-35 `.CRT$X*` slot / cycle-39 EEPROM-scratchpad path / cycle-39 sticky-flag-Codex-round-2 P1 gate: PRESERVED. **Outcome = G0(c) PERSISTS**: post-chainload EEPROM byte at 0xFF = `0xA4` + `witness.scan-self count=0` + `witness.scan = D-cycle-27` shape unchanged. `MmAllocateContiguousMemoryEx(0x1000, 0x10000, 0x3ffffff, 0x1000, PAGE_READWRITE | PAGE_NOCACHE)` STILL did NOT yield a usable allocation on this real-Xbox kernel. Dashboard FTP recovery at t+8s post-chainload (cycle-40 was t+6s; +2s within sampling variance; still consistent with watchdog hardware reset rather than `HalReturnToFirmware` graceful exit).

**Rationale.** The cycle-39 closeout / cycle-40 closure pre-recorded the cycle-41 candidate list ranked low-scope-first: (1) cache-policy `PAGE_NOCACHE`, then `PAGE_WRITECOMBINE`; (2) broaden address range; (3) drop alignment; (4) fall back to non-`-Ex` `MmAllocateContiguousMemory`. Cycle 41a takes the lowest-scope first candidate. Precedent for `PAGE_READWRITE | PAGE_NOCACHE` against `MmAllocateContiguousMemoryEx` exists in nxdk itself at `nxdk/lib/usb/libusbohci_xbox/usbh_xbox.c:35-41` for the OHCI DMA-coherent ring buffers — so the protect bit is real and accepted on stock kernels in at least one nxdk site. Cycle 41a establishes whether the cycle-29 allocation tuple's failure was specifically a cache-policy issue (i.e. the kernel rejects bare-RW and demands an explicit cache-policy bit).

**What cycle-41a discriminates.** ELIMINATES the candidate "kernel rejects bare-RW for `MmAllocateContiguousMemoryEx`" — adding `PAGE_NOCACHE` did NOT recover the allocation. The cycle-22 leading hypothesis is FURTHER STRENGTHENED but not yet narrowed beyond cycle-40: at least one of {`PAGE_WRITECOMBINE` cache-policy, address-range floor/ceiling combination, page-alignment requirement, the `-Ex` variant itself} is the actual failing constraint. Cycle 41a's value is that it KO'd the lowest-scope hypothesis cheaply (1-line code change + 1 rebuild + 1 redeploy + 1 run) before moving onto larger-scope variations.

**Codex review.** mode=`changes`; verdict=MINOR ISSUES. P1 medium "comment overstates kseg0+NOCACHE alias semantics" ADOPTED in spirit via in-code comment tightening (`xbed_self_witness.c:127-145`) — the cycle-41a comment now explicitly frames the slice as ALLOCATOR-ACCEPTANCE triage and acknowledges that the `phys | 0x80000000` cached-mirror alias is unchanged. Codex's alternative suggestion to add a `phys | 0xB0000000` end-to-end uncached-alias variant was DEFLECTED as out-of-slice scope (cycle-41a is one bounded variation; the alias-change is a separate candidate that can be funded later if cache-policy variations exhaust without flipping the regression gate). P2 low "header doc still says RW protection matches agent" ADOPTED via `xbed_self_witness.h:101-118` Safety-notes block rewrite documenting the deliberate cycle-41a NOCACHE divergence + the nxdk OHCI precedent. Open question about `PAGE_WRITECOMBINE` resolved by Codex itself ("the more natural later experiment for NV2A-facing buffers, not an obvious requirement for this specific witness page") — filed as cycle-41b candidate. Out-of-scope finding "agent's own buffer allocator at `oracle-agent/controller.c:221-248` also uses plain `PAGE_READWRITE`" NOTED but not actioned — the agent's buffer empirically works on this kernel-state across many chainloads since cycle 23, so it is not in the failing kernel-state region. Codex marker at `.claude/state/codex-validate-last-run` refreshed (rule #15 compliance).

**Deployment.** Identical to the cycle-40 11-step runbook (reboot → wait dashboard FTP → FTP-upload witness-only XBE with `--overwrite` → `ensure-agent` → `unsafe.enable` + `eeprom.scratch.reset` → confirm `eeprom.scratch.read=0x00` baseline + `witness.scan-self count=0` + `witness.scan = D-cycle-27` → `runxbe path=E:\Apps\witness-only\default.xbe` → poll for dashboard FTP recovery → `ensure-agent` again → final scans + EEPROM dump cross-check). Composite capture SKIPPED with the cycle-34 + cycle-36 ffmpeg silent-stall rationale; cycle-41a primary + secondary signals are fully agent-side and reliable. New cycle-41a witness-only SHA = `7dae8cf9cc08c60f699628eb284a0b1e93b7d8ac580af6851b55b3ed6bb08f78` (155 648 B; oracle-agent unchanged at cycle-39 v0.5 SHA `d419b452…`).

**Recommended cycle 41b.** `PAGE_WRITECOMBINE` cache-policy candidate — symmetric one-line change in `xbed_self_witness.c:138`. If WRITECOMBINE also fails identically (`byte=0xA4 + count=0 + fast watchdog reset`), cache-policy variations are exhausted → cycle 41c broadens to address-range variations (`lowest=0`, `highest=0xFFFFFFFF`), then alignment (cycle 41d: `alignment=0`), then non-`-Ex` fallback (cycle 41e: `MmAllocateContiguousMemory(0x1000)`).

**Reproducibility shape.** `phys=0x03eb3000` deterministic kernel-pool reuse REPRODUCED across baseline + post-deployment + final readbacks (21+ consecutive observations across cycles 26..41a). `mapped_pages_seen=419` REPRODUCED at every readback.

---

## 2026-05-24 (cycle 40 real-Xbox EEPROM scratchpad discriminator run — CLOSED on `apple-silicon-performance`): OUTCOME G0(c); cycle 22 narrowed to `MmAllocateContiguousMemoryEx` returns NULL silently on real-Xbox kernel; cycle 41 explores allocation-flag variations

**Decision.** Cycle 40 executes the cycle-39 closeout's recommended real-Xbox deployment using the 11-step runbook from `witness-only/README.md` cycle-39 addendum (with two minor deviations: composite-record.sh SKIPPED due to cycle-34 + cycle-36 reproduced ffmpeg silent-stall, and the witness-only XBE upload required a 2nd reboot to release dashboard FTP since the cycle-39 oracle-agent v0.5 had already been launched at that point — final state matches the runbook intent). **Outcome = G0(c)**: EEPROM byte at offset 0xFF = `0xA4` (tag=0xA, stage_nib=0x4 — the cycle-39 "pre-MmAlloc self-witness breadcrumb") + `witness.scan-self count=0` + `witness.scan = D-cycle-27`. The cycle-39 EEPROM-write code DID execute on real Xbox; the subsequent `MmAllocateContiguousMemoryEx(0x1000, 0x10000, 0x3ffffff, 0x1000, PAGE_READWRITE)` call did NOT yield a usable allocation (returned NULL silently or crashed inside, leaving `s_witness_page` NULL). **Sub-cases G0(a) "pre-`.CRT$X*` startup crash" and G0(b) "helper body crash before pre-MmAlloc instruction" are ELIMINATED.** The cycle-22 leading hypothesis is FURTHER NARROWED from "pre-main crash, anywhere" to specifically "`MmAllocateContiguousMemoryEx` returns NULL silently on real-Xbox kernel for the cycle-29 allocation tuple".

**Rationale.** The cycle-39 closeout had pre-recorded two branching cycle-40 outcomes with their respective cycle-41 next steps: (G0(c)) → "cycle 41 explores `MmAllocateContiguousMemoryEx` allocation-flag variations"; (G0(a)+(b)) → "cycle 41 needs a custom XBE-header callback (significantly higher scope)". Cycle 40 cleanly lands on G0(c), which is the lower-scope branch — cycle 41 can iterate on small allocation-flag changes (cache policy, address-floor, alignment, or fallback to non-`-Ex` variant) against the cycle-40 G0(c) signal as a regression gate (EEPROM byte should stay `0xA4`; a successful variation should advance `witness.scan-self count >= 1`).

**Anomalously fast dashboard recovery (t+6s vs cycle-36 t+38s) interpretation.** Cycle 36's clean dashboard recovery took ~38s, consistent with a chainload that ran `main()` to completion (cycle-31 paint sequence + cycle-23 fires + cycle-29 in-main fires + `Sleep(2000)` + `HalReturnToFirmware(HalRebootRoutine)` graceful exit). Cycle 40's recovery at t+6s is much faster than any graceful-exit path could be (the `Sleep(2000)` alone is 2s) — strong evidence that the failure happens so early in the cycle-29 first-call branch that nxdk's normal exception-handling / firmware-return path is bypassed entirely, with the Xbox kernel detecting the allocation crash and triggering a watchdog hardware reset → fast BIOS boot → dashboard. This is consistent with G0(c) (allocation-internal crash rather than NULL-return) but does not definitively split "NULL-return" from "kernel-internal crash" — that further split is only achievable via a successful cycle-41 allocation-flag variation (which would either complete normally or fail in a different observable way).

**Composite-capture skip rationale.** Cycle 34 + cycle 36 both reproduced `composite-record.sh` ffmpeg silent-stall (TCC inheritance asymmetry between the cycle-33 preflight and the subsequent long-form ffmpeg recording; finding (i) of cycle 36 secondary findings, upgraded from "one-off cycle-32 + cycle-34" to "reproducible across power sessions"). The cycle-40 primary signal is the EEPROM byte (fully agent-side via TCP/9001, no TCC dependency, no video-pipeline dependency); the secondary signal is `witness.scan-self count` (also fully agent-side). Composite stripes would only matter for G1..G4 outcomes which require `count >= 1` (cycles 26..36 evidence consistently shows count=0). Bounded slice — skip the unreliable capture path; accept the small information loss in exchange for cleaner cycle execution.

**Cycle-40 G-row table reference (from `witness-only/README.md` cycle-39 addendum):**

| EEPROM 0xFF | scan-self count | scan-self reserved1 | scan shape | G-row | Interpretation |
|---|---|---|---|---|---|
| `0x00` | 0 | n/a | `D-cycle-27` | G0(a)+(b) | EEPROM write never executed → crash strictly before `xbed_self_witness_fire` reached its pre-`MmAlloc` instruction. cycle-40 cannot distinguish (a) from (b); cycle 41 would need a pre-`.CRT$X*` callback (high scope). |
| **`0xA4`** | **0** | **n/a** | **`D-cycle-27`** | **G0(c) ← THIS CYCLE'S OUTCOME** | EEPROM write landed → `MmAllocateContiguousMemoryEx` returned NULL silently OR crashed. Sub-cases (a)+(b) ELIMINATED → cycle 41 explores allocation-flag variations. |
...
**Cycle 41 next-step recommendation (binding, pre-recorded by cycle-39 closure).** Cycle 41 explores `MmAllocateContiguousMemoryEx` allocation-flag variations in `lib/xbed_self_witness.c:133-138`. Candidates ranked low → high scope: (1) cache policy `PAGE_READWRITE | PAGE_NOCACHE` and `PAGE_READWRITE | PAGE_WRITECOMBINE`; (2) broaden address range to `lowest=0`, `highest=0xFFFFFFFF`; (3) drop alignment to 0; (4) fall back to `MmAllocateContiguousMemory` (no -Ex). Cycle-40 G0(c) signal IS the regression gate: EEPROM byte at 0xFF must stay `0xA4` across cycle-41 attempts (the sticky `s_eeprom_scratch_attempted` Codex-round-2 P1 fix guarantees this); a successful variation should advance `witness.scan-self count >= 1` with `reserved1` matching the count of fires landed.

**Secondary findings filed for cycle 41+ consideration (NOT cycle-40 scope).** (i) The cycle-39 oracle-agent's `cmd_info` runtime banner string was NOT updated to "v0.5" — `info` still reports "v0.4 (Phase 2 + controller.* + smc.*)" despite the new `eeprom.scratch.*` verbs being present and working. Trivial 1-line fix; out-of-scope for cycle 40 (run-only). (ii) The agent `runxbe` verb requires explicit `path=` named-argument syntax. The cycle-39 README runbook example is correct; the agent's `help` already documents this; no doc change needed but worth tracking because hybrid positional/named argument parsing on the other typed verbs (`mem.*`, `nv2a.*`) can confuse first-pass readers.

**Reproducibility wins.** `phys=0x03eb3000` deterministic kernel-pool reuse + `mapped_pages_seen=419` REPRODUCED across 4 readbacks this session (baseline + post-deploy + post-witness-upload + post-chainload) and 20+ consecutive observations across cycles 26..40. EEPROM non-volatility across two soft reboots within this session CONFIRMED (baseline byte `0x00` survived reboot 2; post-chainload byte `0xA4` survived chainload-induced reset).

**Files changed.** 6 canonical-doc updates (handoff.md cycle-40 entry on top above cycle-39; decision-log.md cycle-40 entry above cycle-39; orchestration-state quartet — current-cycle.md + claude-status.md + validation-status.md + handoff-summary.md — synced to cycle-40 closure). Evidence-only files at `benchmark-runs/cycle40-real-xbox-eeprom-discriminator-20260524T021727Z/{SUMMARY.md, 00-baseline-pre-deploy.txt, 01-reboot.txt, 02-ftp-recovery.txt, 02b-ftp-auth.txt, 03-deploy-agent.txt, 03c-new-verbs-probe.txt, 04-eeprom-baseline.txt, 05-pre-witness-upload-status.txt, 06-pre-witness-upload-reboot.txt, 07-witness-upload-and-baseline-recheck.txt, 08-chainload-and-recovery.txt, 09-runxbe-issue.txt, 10-chainload-recovery.txt, 11-post-run-evidence.txt}` are gitignored per project convention.

**Codex SKIPPED per rule #15 run-only / doc-only carve-out** — ZERO source / script / nxdk / host xemu / lib edits this cycle; the deployed XBE binaries (witness-only SHA `7528bb5b…`, oracle-agent SHA `d419b452…`) are exactly the cycle-39 builds that passed Codex 3-round review at closure commit `33fb5b7e34`. `.claude/state/codex-validate-last-run` marker from cycle 39 remains the relevant marker for the deployed artifacts.

Cycle 40 closes with the G0(c) discriminator answer landed; cycle 41 allocation-flag variation design is Hermes's call.

## 2026-05-23 (cycle 39 EEPROM scratchpad pre-`MmAllocateContiguousMemoryEx` discriminator — CLOSED on `apple-silicon-performance`): ships the cycle-38-recommended Option C dynamic discriminator that uniquely splits G0 sub-case (c) from (a)+(b); Codex 3 rounds (R1 P2 + R2 P1 adopted; R3 deflected); cycle-40 real-Xbox deployment is Hermes's call

**Decision.** Cycle 39 implements the cycle-38 closeout's recommended Option C: an EEPROM scratchpad write at offset `0xFF` inside `xbed_self_witness_fire` AS THE LAST INSTRUCTION before `MmAllocateContiguousMemoryEx`, gated AT MOST ONCE per process via a NEW sticky `s_eeprom_scratch_attempted` flag. Paired with two new oracle-agent verbs `eeprom.scratch.read` and `eeprom.scratch.reset` (the latter gated by the existing `unsafe.enable`). The post-run byte (read on a future cycle-40 real-Xbox run) uniquely discriminates G0 sub-case (c) "`MmAllocateContiguousMemoryEx` returned NULL silently" (byte=`0xA4` + WTNS count=0) from sub-cases (a) "pre-`.CRT$X*` startup crash" + (b) "helper-body crash before reaching the allocation call" (byte=`0x00` + WTNS count=0). All cycle-23 / cycle-29 / cycle-31 / cycle-35 invariants preserved.

**Rationale.** Cycle 38's static-surface analysis confirmed (via lld link-map resolution of `.text 0x16720` as the nxdk-default `_automount_d_drive`) that no witness-only-unique pre-`.CRT$X*` code path exists. The remaining G0 sub-cases (a/b/c) cannot be split further by static analysis — the next genuine information requires dynamic instrumentation. The cycle-38 closeout filed cycle-36 Option C (EEPROM scratchpad) as the lowest-cost option that uniquely discriminates (c) from (a)+(b); other candidates (`out 0xe9` host-log breadcrumb is xemu-only / invisible on real Xbox; `.CRT$XCV` slot between `.CRT$XCU` and `main()` adds no new info beyond cycle-35's stage-4 fire — cycle 36 G0 already shows the earlier `.CRT$XXC` slot did not fire) were enumerated and rejected with reasons. The custom XBE-header callback that would split (a) from (b) requires modifying `nxdk/tools/cxbe/` and is therefore deferred to cycle 40+ only if cycle 39's outcome forces it.

**EEPROM offset selection rationale.** Offset `0xFF` is the highest byte of the 256-byte EEPROM image and lies in the documented 0xC0..0xFF reserved/unused tail of the standard Xbox EEPROM layout (consistently zero on stock OEM consoles — does NOT touch the factory/encrypted region 0x00..0x5F, the user-settings region 0x60..0x96, or the user-settings-checksum region 0x96..0xBF). The 24LC02-class EEPROM endurance is ≥1M write cycles; one byte/run is bounded across hundreds of debugging sessions. Reset to `0x00` is explicit via the new agent verb (gated by `unsafe.enable`) — Hermes invokes it BEFORE each cycle-40+ run to establish a known clean baseline.

**Fire-path gate rationale (Codex round-2 P1 adopted).** Initial design gated the EEPROM write on the existing `s_witness_page == 0` first-call branch. Codex round-2 caught the discriminator-breaking bug: in the exact G0(c) sub-case this targets, allocation fails on the first call → `s_witness_page` stays NULL → later `.CRT$XCU` (stage=5) and in-main (stage=1/3) fires re-enter the first-call branch and OVERWRITE the breadcrumb byte from `0xA4` → `0xA5` / `0xA1` / `0xA3`. Fix: a NEW sticky `s_eeprom_scratch_attempted` static flag set BEFORE the write (not only on `NT_SUCCESS`) preserves the first-stage breadcrumb regardless of allocation outcome.

**Reader semantics (Codex round-1 P2 adopted).** The `cmd_eeprom_scratch_read` verb originally aliased ALL `0xA?` values onto sub-case (c). Codex round-1 caught that the cycle-40 G-row table classifies non-`0xA4` TAG-nibble values as "indeterminate" (rerun with `eeprom.scratch.reset` baseline). Fix: 4-branch decode — `0x00` = cleared baseline → sub-case (a)+(b) if POST-run; `0xA4` = cycle-39 breadcrumb → sub-case (c) if count=0; other `0xA?` = indeterminate (TAG nibble matches but stage_nib != 4); else = indeterminate (foreign write or stale value).

**Scope.** ENTIRELY contained in 5 source files + 2 paired docs:
1. `lib/xbed_self_witness.h` — 3 new `#define`s + ~85-line cycle-39 head-comment addendum.
2. `lib/xbed_self_witness.c` — 1 new sticky-flag static + ~50-LOC code block inside existing first-call branch BEFORE `MmAllocateContiguousMemoryEx`.
3. `oracle-agent/commands.c` — 2 new command implementations + 2 new help lines + 2 new `#define`s.
4. `oracle-agent/commands.h` — 2 new function declarations.
5. `oracle-agent/main.c` — 2 new dispatch-table entries + banner-comment addendum.
6. `witness-only/README.md` — cycle-39 addendum (~120 LOC: scratchpad-contract table + cycle-40 G-row discriminator table + cycle-40 deployment runbook + design-choice rationale).
7. `witness-only/manifest.json` — title extended + `real-xbox/physical/cycle-40` expected_results section.

`witness-only/main.c` UNCHANGED — the EEPROM write is reached automatically through the cycle-35 `.CRT$X*` slot path that calls `xbed_self_witness_fire`. `witness-only/Makefile` UNCHANGED.

**Build.** Both XBEs rebuilt cleanly with the cycle-29 source linked: `witness-only/bin/default.xbe` 155 648 B (unchanged size; new SHA-256 `7528bb5bf4c9cc8f00c934c38e015166de7a6f89236ddc8e2ad2195a47ce4e0d`); `witness-only.iso` 720 896 B (unchanged size; new SHA-256 `d2c2089913267215f51d5fb24d050c20482e859cda72cf6a7c9370fb7e626bd4`); `oracle-agent/bin/default.xbe` 417 792 B (unchanged size; new SHA-256 `d419b452f127e5a065a51b2d5bba49b6f2ec1f1826cdec57845793c34440cb53`); `oracle-agent/oracle-agent.iso` 983 040 B (unchanged size; new SHA-256 `283b11d3b201eb8ff1480e602c2701314fdd3ea7d6163792a7d3dbeeaa2d9cca`). ZERO new lld warnings (benign `.edata=.rdata: already merged into .edataxb` repeats prior cycles).

**Local validation.** Cold-boot xemu smoke (`dist/xemu.app/Contents/MacOS/xemu -config_path <tmp> -display none -nographic` against `witness-only.iso` with `XEMU_GUEST_LOG=1`) did NOT reach the XBE within the bounded slice's ~120 s wait budget — cold boot through BIOS to DVD load via the .app wrapper exceeds that envelope without a pre-warmed snapshot path (the cycle-35 closure's claimed "12 s spawn" must have depended on a snapshot mechanism not available in this bounded session). Structural correctness coverage rests on: (i) clean nxdk lld link of both XBEs with ZERO new warnings; (ii) the cycle-29 shim's existing Codex-validated first-call branch (where the new EEPROM write is inserted); (iii) xemu's QEMU `smbus-eeprom` device implementing both `eeprom_receive_byte` and `eeprom_write_data` per `hw/i2c/smbus_eeprom.c:52-83` (the SMBus write IS honored in emulation when reached, ruling out "broken in xemu" failure mode); (iv) Codex 3-round source review.

**Codex validation (rule #15, mandatory — non-trivial diff > 30 LOC in apple-silicon scripts; ~975-line aggregate diff including ~85 LOC `lib/` source + ~90 LOC agent source + ~118 LOC README + ~6 LOC manifest):**

- **Round 1 = MAJOR ISSUES (1 P2 adopted).** `cmd_eeprom_scratch_read` aliased all `0xA?` values onto sub-case (c); fixed to 4-branch decode that splits `0xA4` from other-`0xA?`.
- **Round 2 = MAJOR ISSUES (1 P1 adopted).** EEPROM write was re-entered on later fires when allocation failed; fixed via NEW sticky `s_eeprom_scratch_attempted` flag set BEFORE the write so it fires AT MOST ONCE per process regardless of allocation outcome.
- **Round 3 = MAJOR ISSUES (1 P1 DEFLECTED — out of cycle-39 scope).** Codex flagged the pre-existing tracked drift in `retail-*.py` (3 files) importing untracked `composite_preflight.py`. This finding is correct on its merits but is about files explicitly preserved unstaged per the cycle-34..38 Hermes-supervision guardrail; the cycle-39 closing commit does NOT stage those files. The cycle-39 source surface itself had no findings in round 3. DEFLECTED with documented reason; responsibility for resolving the tracked-drift dependency belongs to whichever future cycle commits those 4 files.
- Codex marker written at `.claude/state/codex-validate-last-run` with the round-3 fingerprint.

**Hypothesis state.** Three live G0 sub-cases remain (carried over from cycle 38, with cycle-39 instrumentation now in place to discriminate (c) from (a)+(b) on cycle-40+ real-Xbox readback). Cycle-22 pre-main-crash hypothesis remains FULLY CORROBORATED; γ.1 remains INVALIDATED. No new hypothesis introduced this cycle.

**Cycle-40 next step (Hermes's call).** Deploy the cycle-39 oracle-agent + witness-only XBE on real Xbox via the cycle-40 11-step runbook (steps 3a `eeprom.scratch.reset` baseline + 11 `eeprom.scratch.read` recovery bracket the existing cycle-36 chainload window). Outcome G0(c) (`byte=0xA4 + count=0`) → cycle 41 explores allocation-flag variations. Outcome G0(a)+(b) (`byte=0x00 + count=0`) → cycle 41 needs a custom XBE-header callback (high scope; only if forced).

**Out-of-scope (explicit; for evidence of bounded slice).** ZERO host xemu source touched (`hw/`, `ui/`, `target/`, etc. untouched); ZERO `lib/xbed_a4_witness.{c,h}` touched (cycle-23 lockstep contract intact); ZERO `lib/lib.mk` touched; ZERO `lib/xbed_runtime.{c,h}` touched; ZERO `oracle-agent/{controller,smc,tier2,protocol}.{c,h}` touched; ZERO image-blit / pipeline-smoke / mirror / other diag XBE source touched; ZERO `witness-only/main.c` touched (the new code is picked up automatically via the cycle-35 `.CRT$X*` slot → cycle-29 shim path); ZERO `witness-only/Makefile` touched; ZERO `tools/xemu-capture/` source touched; ZERO `composite-record.sh` / `composite-preflight.sh` source touched; ZERO `nxdk/` source touched; ZERO flag default flips; ZERO M15 movement; ZERO retail-title / §G.5 / RT-as-texture / second-wave-XBE work; ZERO real-Xbox run; ZERO PushNotification (bounded implementation slice; cycle-40 outcome may warrant one if it lands a clean G0(c) vs G0(a)+(b) split). Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` PRESERVED unstaged per cycle-34+ guardrail (carried forward); pre-existing untracked `composite_preflight.py` PRESERVED unstaged; 17+ pre-existing untracked `.hermes_*.txt` + `.hermes_launch_*.sh` files at repo root preserved un-staged per the same pattern.

**Supersedes / supplements.** Supplements cycle 38 closure (`1127cafa0d`) by executing its filed recommendation. Does NOT supersede any prior cycle's source; all earlier slices' Codex validations remain in force. Cycle-29 marker at `.claude/state/codex-validate-last-run` updated to the cycle-39 fingerprint (the new fingerprint covers the lib/ + agent + paired docs changes).

## 2026-05-23 (cycle 38 lld link-map symbol resolution for `.text 0x16720` — CLOSED on `apple-silicon-performance`): concrete positive result — `.text 0x16720` = `_automount_d_drive` (nxdk default); cycle-37 F5 "+1 walker group" REVISED to heuristic artifact; cycle-39 recommendation = cycle-36 Option C EEPROM scratchpad write

**Decision.** Cycle 38 executes the cycle-37 closeout's recommended next step (re-link witness-only with lld `--print-map` and resolve which symbol corresponds to the unexplained `.text 0x16720` singleton fn-ptr) as a bounded tooling-only slice. ONE rebuild (witness-only only) with `LDFLAGS=-map:<path>` appended; identical `.obj` inputs; resulting binary structurally bit-identical to cycle-35 modulo 8 timestamp bytes total (verified via `cmp -l`); tracked artifacts restored to cycle-35 deployed bytes from git HEAD post-rebuild. ZERO source / script / nxdk / host xemu / lib edits.

**Result (concrete positive).** `.text 0x16720` = **`_automount_d_drive`** in `libnxdk_automount_d:automount_d.obj`. Source: `nxdk/lib/nxdk/automount_d.c:35` registers `automount_d_drive_p` in `.CRT$XIT` via `__attribute__((section(".CRT$XIT"), used))`. This is an **nxdk default**, force-included by `nxdk/lib/nxdk/Makefile:38-41` for every nxdk XBE that does NOT set `NXDK_DISABLE_AUTOMOUNT_D=y`. Neither witness-only's Makefile, lib.mk, mirror's Makefile, nor pipeline-smoke's Makefile sets that flag — so all three nxdk XBEs include `_automount_d_drive`. Cycle-37's directionally-correct candidate-source guess (`automount_d.obj`) is **confirmed** as the source, but cycle-37's underlying assumption that this would represent a witness-only-unique drag-in is **FALSE**.

**Cycle-37 F5 hypothesis REVISED.** The "+1 unexplained walker group" was a heuristic-counting artifact of how the cycle-37 Python XBE parser delineated zero-bounded fn-ptr runs in the merged `.CRT=.rdata` region. The actual semantic CRT walker layout (from the lld map) is: `.CRT$XCA..XCZ` walker = 1 entry (`_witness_only_pre_main_crt_xc` from cycle 35); `.CRT$XIA..XIZ` walker = 1 entry (`_automount_d_drive`, nxdk default); `.CRT$XXA..XXZ` walker = 3 entries (`_witness_only_pre_main_crt_xx` from cycle 35; `_fls_init` from `libwinapi:fiber.obj`; `_tsc_freq_init` from `libwinapi:profiling.obj`). The +2 cycle-35 entries (XCU + XXC) are the ONLY semantic walker delta vs a baseline non-witness nxdk XBE; the +1 walker group cycle-37 reported was layout/heuristic, not real.

**Cycle-37 dump entries past `.CRT$XXZ` are not walker entries.** VAs `0x3303c` onward resolve to `.text` addresses inside `libpdclib:malloc.obj` (`_dlmalloc_set_footprint_limit + 100`, `_internal_mallinfo + 256`, `_internal_mallinfo + 572`). Most likely `.rdata`-resident pdclib malloc constants / vtables, NOT pre-main initializer callbacks.

**Hypothesis state after cycle 38.** γ.0 narrowed further: combined with cycle-37 F1/F2/F3/F4, the static binary surface of `witness-only` is now confirmed structurally identical to mirror modulo the 2 cycle-35 walker entries. **No witness-only-unique pre-`.CRT$X*` code path exists.** Three live G0 sub-cases remain (cycle-37 F5 "+1 walker group drag-in" alternative ELIMINATED): (a) crash in nxdk's pre-`.CRT$X*` startup (`_start` / `__security_init_cookie` / TLS-size / `_PDCLIB_xbox_libc_init`); (b) crash inside `_witness_only_pre_main_crt_xx`'s body BEFORE `MmAllocateContiguousMemoryEx`; (c) `MmAllocateContiguousMemoryEx` returns NULL silently. Cycle-22 leading hypothesis remains FULLY CORROBORATED. γ.1 remains INVALIDATED (cycle 36 G0 outcome).

**Recommended cycle-39 next step (lowest-cost, highest-information).** The static surface has now been exhausted as a discriminator for the 3 remaining G0 sub-cases. Recommended cycle-39: cycle-36 Option C (EEPROM scratchpad write inside `xbed_self_witness_fire` BEFORE `MmAllocateContiguousMemoryEx`, paired with agent EEPROM read-back). Uniquely discriminates (c) from (a)+(b). Other candidates considered and rejected (alternatives in handoff cycle-38 entry): `out 0xe9` host-log breadcrumb (invisible on real Xbox — SKIP); `.CRT$XCV` slot between XCU and main() (adds no new info beyond cycle-35 stage-4 fire — SKIP).

**Why Codex is skipped.** Slice did not produce any non-trivial code change. The one rebuild was a tooling-only `LDFLAGS=-map:<path>` add and the resulting deployed-binary bytes were restored to the cycle-35 Codex-validated artifact. No source diff in xemu-fork apple-silicon scripts beyond doc/state updates. Rule #15 trivial-work carve-out applies. The cycle-35 marker at `.claude/state/codex-validate-last-run` remains the relevant marker for the deployed artifact (closure commit `515e03f4e7`).

**Why this does not invalidate decisions from cycles 22-37.** Cycle 35's `.CRT$X*` design choice remains correct: the slots landed correctly in the binary AND the cycle-38 lld map confirms the cycle-35 helpers are positioned exactly between the sentinels (`.CRT$XCU` between `XCA`/`XCZ`; `.CRT$XXC` between `XXA`/`XXZ`/`XXT`). Cycle 36's G0 classification stands: the static surface still cannot distinguish G0 sub-cases (a)/(b)/(c). Cycle 37's F1-F4 findings are unchanged (XBE format, TLS, kernel imports, cycle-35 slot presence — all confirm witness-only is structurally well-formed and import-compatible with mirror). Cycle-23 lockstep contract is intact (`lib/xbed_a4_witness.h` untouched). Cycle-37 F5 is the only finding that gets revised — from "+1 unexplained walker group + +2 fn-ptrs" to "no semantic walker delta beyond cycle 35's intended 2 slots."

**Files.** 6 canonical-doc updates (handoff.md cycle-38 entry on top above cycle-37; decision-log.md cycle-38 entry above cycle-37; orchestration-state quartet — current-cycle.md + claude-status.md + validation-status.md + handoff-summary.md — synced to cycle-38 closure). Evidence-only files at `benchmark-runs/cycle38-link-map-symbol-resolution-20260524T000443Z/{SUMMARY.md, witness-only.map, 04-crt-walker-symbol-resolution.txt, 05-reproducibility-evidence.txt, main.exe.cycle35-original, default.xbe.cycle35-original}` are gitignored per project convention. Pre-existing tracked drift in 4 `scripts/apple-silicon/*.{sh,py}` files PRESERVED unstaged per the rolling Hermes-supervision pattern (carried forward from cycle 37); pre-existing untracked `scripts/apple-silicon/composite_preflight.py` PRESERVED unstaged; 14+ pre-existing untracked `.hermes_*.txt` + `.hermes_launch_*.sh` files at repo root preserved un-staged per the same pattern.
