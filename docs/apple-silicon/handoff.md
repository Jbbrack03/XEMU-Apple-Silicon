...
Last updated: 2026-05-23 (cycle 37 static XBE binary diff investigation — **CLOSED on `apple-silicon-performance`**; analysis-only slice; narrowed-but-not-conclusive negative result: witness-only's XBE header / TLS / kernel-import surface is structurally identical to known-good mirror; one new structural drift surfaced: +1 unexplained `.CRT$X*` walker group with a singleton fn-ptr at `.text 0x16720` vs mirror; cycle 38 recommendation: re-link with lld `--print-map` to resolve which symbol lives at that address. Cycle 36 + Cycle 35 entries preserved unchanged below).
