...
Last updated: 2026-05-23 (cycle 36 real-Xbox discriminator run for cycle-35 pre-main breadcrumb — **CLOSED on `apple-silicon-performance`**; outcome **G0** = no stripes + `witness.scan-self count=0` + `witness.scan D-cycle-27`, REPRODUCED across two runxbe attempts in same physical power session; pre-`.CRT$XXC` window — strictly earlier than cycle-34's F4. Cycle 35 entry preserved unchanged below).
