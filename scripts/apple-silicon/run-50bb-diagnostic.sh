...
RUN_DIR="benchmark-runs/50BB-hot-path-depth-state-pgr2"
mkdir -p "$RUN_DIR"
...
# 50BB: diagnostic run focused on hot-path depth-stencil state application
# for bound surface 0x038e0000 near screenshot capture time
export XEMU_NATIVE_TRI_DEPTH=1
export XEMU_METAL_RTT_SIBLING_SYNC_DEPTH=1
export XEMU_METAL_DIAG_DEPTH_STATE=1
export XEMU_METAL_DIAG_DEPTH_CAPTURE=1
...
echo "=== 50BB DIAG: collecting output ==="
echo ""
echo "=== 50BB DIAG lines (depth state on every apply) ==="
grep "50BB DIAG" "$RUN_DIR/xemu.log" 2>/dev/null || echo "(no 50BB DIAG lines found)"
echo ""
echo "=== 50BB lines for bound 0x038e0000 ==="
grep "50BB DIAG.*0x038e0000" "$RUN_DIR/xemu.log" 2>/dev/null || echo "(no 50BB DIAG lines for 0x038e0000)"
echo ""
